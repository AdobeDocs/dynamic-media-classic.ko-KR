...
계정 생성 시 Dynamic Media Classic은 회사에 할당된 서버를 자동으로 제공합니다. 이 서버는 웹 사이트와 애플리케이션의 URL 문자열을 구성하는 데 사용됩니다. 이러한 URL 호출은 계정과 관련이 있습니다.
...
[이미지 사전 설정] 화면은 이미지 사전 설정을 만들고 편집하는 데 사용됩니다. Dynamic Media Classic에서는 이미지 사전 설정을 사용하여 동일한 마스터 이미지와 다른 크기로 이미지를 동적으로 제공할 수 있습니다. 각 이미지 사전 설정은 이미지 표시를 위한 크기 및 형식 지정 명령의 사전 정의된 모음을 나타냅니다. 이미지 사전 설정을 만들 때 이미지 제공 크기를 선택합니다. 또한 이미지를 제공할 때 이미지 모양이 보기에 최적화되도록 형식 지정 명령을 선택합니다.
...
고급 [이미징을 참조하십시오](https://helpx.adobe.com/experience-manager/6-3/assets/using/imaging-faq.html).
...
**사전 설정 이름** 빈 공간 없이 설명형 이름을 입력합니다. 사용자가 이 이미지 사전 설정을 식별하기 쉽도록 이미지 크기 사양을 이름에 포함합니다.

**너비 및 높이** 이미지가 전달되는 크기의 픽셀을 입력합니다.

**서식** 메뉴에서 형식을 선택합니다. GIF, JPEG, PDF 또는 TIFF 형식을 선택하면 다음과 같은 추가 선택 사항이 표시됩니다.
...
**선명하게 하기** 모든 크기 조정 후 이미지에 기본 선명하게 하기 필터를 적용하려면 [단순 선명하게 하기 활성화] 옵션을 선택합니다. 선명하게 하기는 이미지를 다른 크기로 표시할 때 발생할 수 있는 흐림을 보상하는 데 도움이 됩니다. 
...
**금액** 가장자리 픽셀에 적용된 대비 양을 제어합니다. 기본값은 1.0입니다. 고해상도 이미지의 경우 5.0만큼 높게 늘릴 수 있습니다. 이 양을 필터 강도의 측정값으로 간주합니다.

**반경** 선명하게 하기에 영향을 주는 가장자리 픽셀 주위의 픽셀 수를 결정합니다. 고해상도 이미지의 경우 1에서 2 사이의 값을 입력합니다. 값이 작으면 가장자리 픽셀만 선명하게 되고 값이 크면 넓은 범위의 픽셀이 선명하게 됩니다. 올바른 값은 이미지 크기에 따라 달라집니다.
...
**색상 공간** 일반적으로 RGB(원본) 또는 광도 공간(강도)으로 이미지가 만들어진 공간을 이미지에 사용할지 여부를 결정합니다.
...
**렌더링 의도** 색상 프로파일의 기본 렌더링 의도를 무시하려면 옵션을 선택합니다. 기본 ICC 프로필 중 하나가 색상 전환의 타겟 색상 공간이고, 출력 장치(프린터 또는 모니터)가 이 프로필을 사용하며, 지정한 렌더링 의도가 이 프로필에 유효한 경우 이 선택 사항을 사용합니다.
...
* 다음 적응형 비디오 사전 설정이나 단일 인코딩 사전 설정 중에서 선택합니다.
   ...
   **16:9 적응형 비디오**
   ...
   뷰어 연결 속도에 가장 적합한 해상도와 비트 전송률에 맞게 최적화된 데스크탑, 모바일(iPhone, iPad, Android) 및 태블릿(iPad, Android)에 전달할 16:9 화면비 비디오를 제작할 수 있습니다.

   **4:3 적응형 비디오**

   뷰어 연결 속도에 가장 적합한 해상도와 비트 전송률을 고려하여 최적화된 데스크탑, 모바일(iPhone, iPad, Android) 및 태블릿(iPad, Android)에 전달할 4:3 화면비 비디오를 제작할 수 있습니다.
   ...
   **적응형 비디오 인코딩(16:9 또는 4:3)**
   ...
   뷰어 연결 속도에 가장 적합한 해상도와 비트 전송률을 고려하여 최적화된 데스크탑, 모바일(iPhone, iPad, Android) 및 태블릿(iPad, Android)에 전달할 16:9 및 4:3 화면비 비디오를 모두 만들 수 있습니다.
...
**인코딩된 파일 접미사** 식별을 위해 인코딩된 비디오 파일에 첨부된 접미어입니다.
...
**오디오 비트 전송률** 1초의 오디오 재생을 구성하기 위해 인코딩되는 데이터의 양(kbit/초)입니다.
...
| 2 | 4:3, 384x288px, 모바일(iPhone, iPad, Android), (400 Kbps) | 500 | _Mobile_384x288_400K | 400 | 384x288 | 소스와 동일 | 64 | 저해상도, 3G |
| 3 | 16:9, 512x288, 모바일(iPhone, iPad, Android), (600 Kbps) | 700 | _Mobile_512x288_600K | 600 | 512x288 | 소스와 동일 | 64 | 중해상도, 3G |
| 4 | 4:3, 384x288, 모바일(iPhone, iPad, Android), (600 Kbps) | 700 | _Mobile_384x288_600 | 600 | 384x288 | 소스와 동일 | 64 | 중해상도, 3G |
| 5 | 16:9, 640x360, 태블릿(iPad, Android), (800 Kbps) | 900 | _iPad_640x360_800K | 800 | 640x360 | 소스와 동일 | 80 | 중해상도, WiFi |
| 6 | 4:3, 640x480, 태블릿(iPad, Android), (800 Kbps) | 900 | _iPad_640x480_800K | 800 | 640x480 | 소스와 동일 | 80 | 중해상도, WiFi |
| 7 | 16:9, 768x432, 태블릿(iPad, Android), (1200 Kbps) | 1.5Mbps | _iPad_768x432_1200K | 1200 | 768x432 | 소스와 동일 | 96 | 고해상도, WiFi |
| 8 | 4:3, 768x576, 태블릿(iPad, Android), (1200 Kbps) | 1.5Mbps | _iPad_768x576_1200K | 1200 | 768x576 | 소스와 동일 | 96 | 고해상도, WiFi |
| 9 | 16:9, 1280x720, 데스크톱, (2000Kbps) | 3.0Mbps | _1280x720_2000K | 2000 | 1280x720 | 소스와 동일 | 128 | 고화질, 와이드스크린 |
| 10 | 4:3, 1280x960, 데스크톱, (2000Kbps) | 3.0Mbps | _1280x960_2000K | 2000Kbps | 1280x960 | 소스와 동일 | 128 | 고화질 |
...
**견본 집합 뷰어** 이미지를 다른 색상, 재료, 텍스처, 마무리 또는 천으로 표시합니다. 사용자가 썸네일을 클릭하여 이미지의 변형을 확인합니다.
...
**스핀 세트 뷰어** 사용자가 개체를 돌려 다른 측면과 각도를 검사할 수 있도록 이미지의 여러 보기를 제공합니다.
...
**확대/축소 뷰어** 세 가지 유형의 확대/축소 뷰어를 제공합니다.
...
|--- |--- |--- |--- |--- |--- |--- |--- |--- |
| Universal_HTML5_Video(자막 지원 포함) See [Best practice: Using the Universal HTML5 Video viewer.](best-practice-using-html5-video.md#best_practice_using_the_html5_video_viewer) | HTML5 | X | X | X | X | X | X | X |
...
**사전 설정 추가** [추가]를 클릭하고 [뷰어 사전 설정 추가] 대화 상자에서 선택을 합니다.

[뷰어 사전 설정 추가 및 편집](application-setup.md#adding_and_editing_viewer_presets)을 참조하십시오.

**사전 설정** 편집 사전 설정을 선택한 다음 **편집을 클릭합니다**.

[뷰어 사전 설정 추가 및 편집](application-setup.md#adding_and_editing_viewer_presets)을 참조하십시오.

**사전 설정** 삭제 사전 설정을 선택한 다음 **삭제를 클릭합니다**.

**사전 설정** 내보내기 HTML5 뷰어 사전 설정을 선택한 다음 내보내기를 클릭하여 뷰어 스킨을 다운로드하여 새 뷰어 사전 설정을 만들고 추가하는 기초로 사용할 수 있습니다.

HTML5 [뷰어 사전 설정 내보내기를 참조하십시오](application-setup.md#exporting_an_html5_viewer_preset).
...
사용자 인터페이스의 [추가]를 사용하여 뷰어 사전 설정을 추가하는 것은 물론 [내보내기]를 사용하여 뷰어 사전 설정을 추가할 수도 있습니다. 기존 HTML5 뷰어 사전 설정을 내보낸 다음 새로운 사전 설정의 기초로 사용하면 됩니다.
...
1. [뷰어 사전 설정] 화면에서 뷰어 사전 설정을 추가하거나 편집합니다.
   ...
   **추가** 도구 모음에서 추가를 클릭합니다. [뷰어 사전 설정 추가] 대화 상자에서 플랫폼을 선택한 다음 리치 미디어 자산 유형을 선택합니다. 
   ...
   **편집** 기존 뷰어 사전 설정을 선택한 다음 **편집을 클릭합니다**.
...
1. 새로운 HTML5 뷰어 사전 설정의 기초로 사용할 뷰어 사전 설정을 선택합니다.
...
1. (Optional) Select **Include UDF** to include user-defined fields in the view. 사용자 지정 필드는 [세부 사항 보기]에서 [메타데이터] 패널 상단에 표시됩니다.
...
1. 다음 중 하나를 수행합니다.
   ...
   * To make a view the default, select it and then click **Make Default**. 기본 보기는 사용자가 [세부 사항 보기]에서 자산을 열고 [메타데이터] 패널로 이동할 때 표시되는 보기입니다.
...
1. [메타데이터 사전 설정] 화면에서 다음 중 하나를 수행합니다.
   ...
   * To create a preset, click **Add**. 메타데이터 템플릿 이름 텍스트 필드에 사전 설정 이름을 입력한 다음 **메타데이터** 보기를 클릭하고 드롭다운 목록에서 보기를 선택합니다( [메타데이터 보기 참조](application-setup.md#metadata_views)).
...
1. [사용자 지정 필드] 대화 상자에서 원하는 선택 사항을 설정합니다.
   ...
   **날짜** A 날짜. MM/DD/YYYY 형식을 사용할 수 있습니다.
   ...
   **기본값** 필드에 사용자가 입력할 가능성이 가장 큰 값을 입력합니다(선택적). 입력한 값은 만든 필드의 기본값이 됩니다.

   **적용 대상** 메타데이터 필드를 특정 자산 유형에만 적용하려는 경우 자산 유형을 선택합니다(선택적).
...
**활성화 필드** 필드 이름 옆에 있는 활성 옵션을 선택하거나 선택 취소하려면 클릭합니다. 회사 관리자 역할인 경우, 이 옵션이 보이지 않을 수 있습니다. 이 옵션은 MediaPortal과 관련되어 있으므로 개인 설정에 미디어 포털 기능 표시를 선택(켜기)하여 활성화 필드를 확인해야 합니다.
...
1. 시퀀스 순서의 경우, 세트가 Dynamic Media Classic에서 함께 그룹화된 후 이미지의 순서를 정의합니다. 기본적으로 자산은 영숫자 순서로 지정됩니다. 그러나 정규 표현식의 쉼표 구분 목록을 사용하여 순서를 정의할 수 있습니다.

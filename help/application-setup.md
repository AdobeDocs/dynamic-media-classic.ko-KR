...
응용 프로그램 일반 설정 페이지를 열려면 전역 탐색 막대에서 **[!UICONTROL 설정]** > **[!UICONTROL 응용 프로그램 설정]** > **[!UICONTROL 일반 설정]**&#x200B;으로 이동합니다.
...
* **[!UICONTROL 게시된 서버 이름]**  - 이 서버는 계정에 대한 모든 시스템 생성 URL 호출에 사용되는 CDN(Live Content Delivery Network) 서버입니다. Dynamic Media Classic 지원 엔지니어가 이를 지시하지 않는 한 이 서버 이름을 변경하지 마십시오.

* **[!UICONTROL 원본 서버 이름]**  - 이 서버는 품질 보증 테스트에만 사용됩니다. Dynamic Media Classic 지원 엔지니어가 지시를 받지 않는 한 이 서버 이름을 변경하지 마십시오.
...
* **[!UICONTROL Test&amp;Target 서버 이름]**  - .com까지 포함된 Test&amp;Domain URL입니다. 이 URL을 가져오는 방법에 대한 지침은 [!DNL Dynamic Media Classic]과 [!DNL Adobe Target Standard/Premium] 통합을 참조하십시오.

<!-- **Test Publish Context Server Name** -->

* **[!UICONTROL iOS 스트리밍 서버 이름]**  -  [!DNL Dynamic Media Classic] iOS 스트리밍 서버의 URL입니다. 이 서버는 HTTP 프로토콜을 사용하여 스트리밍 비디오를 iOS 기반 장치에 제공합니다.

* **[!UICONTROL Progressive Video Server Name]**  -  [!DNL Dynamic Media Classic] 점진적 비디오 서버의 URL입니다. 이 서버는 HTTP 프로토콜을 사용하여 점진적 비디오를 제공합니다.

* **[!UICONTROL 게시되지 않은 자산에 대한 URL 표시]**  - 자산이 게시되었는지 여부 [!DNL Dynamic Media Classic] 에 관계없이 자산을 미리 볼 때 URL을 표시하려면 이 옵션을 선택합니다. 자산이 게시되지 않은 경우 URL이 작동하지 않지만 계획 또는 구성 용도로 URL을 사용할 수 있습니다.
...
* **[!UICONTROL CDN 무효화 템플릿]**  - CDN(Content Delivery Network) 캐시를 무효화하는 데 사용되는 템플릿을 지정합니다.
   ...
   URL 목록 상자에서 **[!UICONTROL 계속]**&#x200B;을 선택하여 해당 특정 이미지 URL 호출에 대한 캐시를 지웁니다. URL 목록 상자에 URL을 입력하거나 붙여넣어 추가할 수도 있습니다. 템플릿을 미리 설정할 필요가 없습니다.
...
* **[!UICONTROL 프로젝트 표시]**  - Dynamic Media Classic 자산을 구성하는 방법으로 프로젝트를 사용할 수 있는지 여부를 결정합니다. 프로젝트를 사용하여 작업 구성을 참조하십시오.
...
* **[!UICONTROL 인코딩된 비디오 표시]**  - 기본적으로 선택되지 않음(해제).
   ...
   동일한 비디오의 인코딩된 다양한 파생을 탐색하지 않고 Dynamic Media Classic에서 비디오를 빠르게 검색하여 찾아보려면 이 옵션을 선택 취소(기본값) 상태로 유지합니다. 기본 비디오 축소판(업로드한 소스 비디오, 파생물을 만드는 데 사용한 소스 비디오)과 &quot;상위&quot; 응용 비디오 세트 축소판(인코딩된 비디오 세트의 &quot;하위&quot; 파생물을 포함함)만 표시됩니다.
   ...
   또한 **[!UICONTROL 파일]** > **[!UICONTROL 재처리]**&#x200B;로 이동하여 응용 비디오 세트에서 직접 인코딩된 &quot;하위&quot; 비디오를 만들 수도 있습니다. Dynamic Media Classic은 자동으로 응용 비디오 세트의 &quot;상위&quot; 기본 비디오를 찾아 코드 변환을 위한 소스 비디오로 사용합니다. 그러나 새 개별 인코딩된 비디오를 저장할 때 검색 또는 탐색 시 해당 비디오는 표시되지 않습니다. 그러나 [세부 사항 보기]의 [인코딩된 비디오] 탭에서 여전히 액세스할 수 있습니다.
   ...
   [작성] 메뉴의 특정 작업은 개별 비디오에서만 작동하거나 선택적으로 작동합니다. 이 기능을 사용하려면 **[!UICONTROL [인코딩된 비디오 표시]]** 설정 방법과 관계없이 선택할 수 있는 모든 인코딩된 비디오 파생을 표시해야 합니다. **[!UICONTROL 인코딩된 비디오 표시]** 설정을 오버라이드하는 빌드 작업에는 **[!UICONTROL 응용 비디오 세트]** 및 **[!UICONTROL eCatalogs]**&#x200B;가 포함됩니다.
...
* **[!UICONTROL 이미지 덮어쓰기]**  - Dynamic Media Classic에서는 두 파일의 이름이 같을 수 없습니다. 각 항목의 Dynamic Media Classic ID(이미지 이름에서 파일 확장명을 뺀 숫자)는 고유해야 합니다. 이 규칙 때문에 [업로드] 대화 상자에는 [덮어쓰기] 선택 사항이 있습니다. 이 선택 사항의 정확한 효과는 지정한 [이미지 덮어쓰기] 선택 사항에 따라 달라집니다. 이러한 선택 사항은 대체 이미지 업로드 방법(원본 이미지를 바꾸는지, 아니면 중복 이미지가 되는지 여부)을 지정합니다. 중복 이미지는 &quot;-1&quot;을 사용하여 이름이 바뀝니다. 예를 들어 chair.tif는 chair-1.tif로 이름이 바뀝니다. 이 선택 사항은 원본과 다른 폴더에 업로드된 이미지나 원본과 다른 파일 이름 확장자(예: JPG, TIF 또는 PNG)를 가진 이미지에 영향을 줍니다. [이미지 덮어쓰기] 선택 사항 사용을 참조하십시오.
   ...
   * **[!UICONTROL 현재 폴더에 덮어쓰기, 확장자와 관계없이 동일한 기본 자산 이름]**  - 대체 이미지를 원본과 동일한 폴더로 업로드해야 하지만 파일 이름 확장명은 원본과 다를 수 있습니다. 예를 들어 chair.tif가 chair.jpg를 대체합니다.

   * **[!UICONTROL 모든 폴더에 덮어쓰기, 동일한 기본 자산 이름/확장명]**  - 대체 이미지의 파일 확장명이 원본 이미지와 동일해야 합니다(예: chair.jpg는 chair.tif가 아니라 chair.jpg로 대체해야 함). 그러나 대체 이미지를 원본과 다른 폴더에 업로드할 수 있습니다. 업로드한 이미지는 새 폴더에 있으며 원래 위치에서 더 이상 파일을 찾을 수 없습니다.
...
* **[!UICONTROL 게시 유지]**  - Dynamic Media Classic에 업로드된 대체 이미지가 대체되는 이미지의 게시 준비 설정을 유지하는지 또는 업로드 시 설정이 지정되는지를 지정합니다.
...
* **[!UICONTROL 기본 이미지 매핑 HREF]**  - 이미지 매핑의 HREF 열에 사용되는 기본 URL을 정의합니다. 이 URL은 이미지 맵을 만들 때 표시되는 기본 URL입니다.

* **[!UICONTROL 기본 이미지 매핑 템플릿]**  - 이미지 매핑에서 HREF 템플릿에 대한 기본 JavaScript를 정의합니다. 이미지 맵을 선택할 때마다 실행되도록 여기에서 사용자 지정 코드를 설정할 수 있습니다.
...
[이미지 사전 설정] 화면은 이미지 사전 설정을 만들고 편집하는 데 사용됩니다. 이미지 사전 설정을 사용하면 Dynamic Media Classic에서 동일한 마스터 이미지와 다른 크기로 이미지를 동적으로 제공할 수 있습니다. 각 이미지 사전 설정은 이미지 표시를 위한 크기 및 형식 지정 명령의 사전 정의된 모음을 나타냅니다. 이미지 사전 설정을 만들 때 이미지 제공 크기를 선택합니다. 또한 형식 지정 명령을 선택하면 이미지를 볼 수 있도록 전달할 때 이미지의 모양을 최적화할 수 있습니다.

관리자는 자산 내보내기에 대한 사전 설정을 만들 수 있습니다. 사용자는 이미지를 내보낼 때 사전 설정을 선택할 수 있으며, 이 경우 관리자가 지정하는 사양에 맞게 이미지를 재포맷할 수도 있습니다.

이미지 사전 설정 화면을 열려면 전역 탐색 막대에서 **[!UICONTROL 설정]** > **[!UICONTROL 이미지 사전 설정]**&#x200B;으로 이동합니다.
...
1. 사전 설정을 만들거나 기존 사전 설정에서 시작합니다.
   ...
   * **기존 사전 설정에서 이미지 사전 설정 작성**  - 만들려는 이미지 사전 설정과 가장 유사한 이미지 사전 설정을 선택한 다음  **[!UICONTROL 편집]**&#x200B;을 선택합니다.
...
1. 자체 이미지로 사전 설정을 미리 보려면 **[!UICONTROL 찾아보기]**&#x200B;를 선택한 다음 이미지를 선택하십시오. 기본 이미지로 미리 보려면 **[!UICONTROL 재설정]**&#x200B;을 선택합니다.

이미지 사전 설정 화면에서 해당 이름을 선택한 다음 **[!UICONTROL 편집]**&#x200B;을 선택하여 이미지 사전 설정을 편집할 수 있습니다. 이미지 사전 설정을 삭제하려면 해당 이미지 사전 설정을 선택한 다음 **[!UICONTROL 삭제]**&#x200B;를 선택합니다.
...
* **[!UICONTROL 형식]**  - 메뉴에서 형식을 선택합니다. GIF, JPEG, PDF 또는 TIFF 형식을 선택하면 더 많은 옵션이 표시됩니다.
   ...
   * GIF 색상 양자화 선택 사항
      ...
      * **[!UICONTROL 색상 수]**  - 슬라이더를 드래그하여 2-255로 입력합니다.
...
* **[!UICONTROL Colorspace]**  - 색상 공간을 선택합니다.
...
* **[!UICONTROL B-선형]**  - 가장 빠른 재샘플링 방법 일부 앨리어싱 가공물들은 눈에 띄습니다.

* **[!UICONTROL Bi-Cubic]**  - 이미지 서버에서 CPU 사용을 증가시키지만, 앨리어싱 가공물이 덜 눈에 띄게 선명해진 이미지를 생성합니다.

* **[!UICONTROL Sharp2]**  - Bi-Cubic 옵션보다 약간 더 선명한 결과를 생성할 수 있지만 이미지 서버에서 CPU 비용이 훨씬 더 높습니다.

* **[!UICONTROL 3선형]**  - 사용 가능한 경우 더 높은 해상도와 더 낮은 해상도를 모두 사용합니다. 앨리어싱이 문제인 경우에만 권장됩니다. 이 방법을 사용하면 고주파수 데이터가 감소하기 때문에 JPEG 크기가 줄어듭니다.

* **[!UICONTROL 언샵 마스킹]**  - 선명도 미세하게 조정하려면 다음 옵션을 선택합니다.

* **[!UICONTROL 금액]**  - 가장자리 픽셀에 적용되는 대비의 양을 제어합니다. 기본값은 1.0입니다. 고해상도 이미지의 경우 5.0만큼 높게 늘릴 수 있습니다. 이 양을 필터 강도의 측정값으로 간주합니다.

* **[!UICONTROL 반경]**  - 선명하게 하기에 영향을 주는 가장자리 픽셀을 둘러싸는 픽셀 수를 결정합니다. 고해상도 이미지의 경우 1에서 2 사이의 값을 입력합니다. 값이 작으면 가장자리 픽셀만 선명하게 되고 값이 크면 넓은 범위의 픽셀이 선명하게 됩니다. 올바른 값은 이미지 크기에 따라 달라집니다.
...
* **[!UICONTROL 출력 색상 프로필]**  - Dynamic Media  **[!UICONTROL Classic]** 에서 사용할 수 있는 ICC 색상 프로파일 중 하나를 선택합니다.

   [ICC 프로필](icc-profiles.md#icc_profiles)을 참조하십시오.

* **[!UICONTROL 렌더링 의도]**  - 색상 프로필의 기본 렌더링 의도를 무시하려면 옵션을 선택합니다. 기본 ICC 프로파일 중 하나가 색상 변환의 대상 색상 공간인 경우 이 옵션을 사용합니다. 또는 출력 장치(프린터 또는 모니터)는 이 프로파일을 나타내며, 지정한 렌더링 의도가 이 프로파일에 유효합니다.

* **[!UICONTROL 포함 프로필]**  - 이 이미지를 Adobe® Photoshop®에서 열면 이 프로필을 사용하도록 이 옵션을 선택합니다.

* **[!UICONTROL 인쇄 해상도]**  - 이 이미지를 인쇄할 해상도를 선택하십시오. 기본값은 72픽셀입니다.
...
1. [이미지 사전 설정] 화면의 테이블에서 사전 설정을 선택하고 다음 중 하나를 수행합니다.
   ...
   * MediaPortal 사용자의 전체 Dynamic Media Classic 사용자 인터페이스에서 해당 이름을 제거하려면 사전 설정 이름 옆에 **[!UICONTROL Active]** 확인란을 선택 취소합니다.
...
1. Dynamic Media Classic의 오른쪽 위 모서리 근처에 있는 **[!UICONTROL 설정]** > **[!UICONTROL 애플리케이션 설정]** > **[!UICONTROL 비디오 사전 설정]** > **[!UICONTROL 응용 비디오 사전 설정]**&#x200B;으로 이동합니다.
...
인코딩 사전 설정을 선택하려면 업로드 페이지의 오른쪽 아래 모서리에서 **[!UICONTROL 작업 옵션]**&#x200B;을 선택합니다. 업로드 작업 옵션 대화 상자에서 비디오 옵션을 확장하고 원하는 비디오 인코딩 사전 설정을 선택합니다.
...
* 다음 응용 비디오 인코딩 또는 단일 인코딩 사전 설정 중에서 선택합니다.
   ...
   * **[!UICONTROL 응용 비디오 인코딩(16:9 또는 4:3)]**  - 데스크톱, 모바일(iPhone, iPad, Android™) 및 태블릿(iPad, Android™)에 전달할 수 있는 16:9 및 4:3 종횡비 비디오를 모두 만듭니다. 모두 뷰어의 연결 속도와 가장 일치하는 해상도 및 비트 전송률로 최적화되었습니다.
...
**[!UICONTROL 업로드 시작]**&#x200B;을 선택하면 원본 마스터 비디오 파일이 업로드되고 마스터 파일에서 인코딩된 파일이 생성됩니다.

### 인코딩 사전 설정 선택 사항 정보 {#about-encoding-preset-options}

인코딩 사전 설정 선택 사항의 매개 변수는 다음과 같습니다.

* **[!UICONTROL Target 연결 속도]**  - 타깃팅된 최종 사용자의 인터넷 연결 속도입니다.

* **[!UICONTROL 인코딩된 파일 접미사]**  - 식별을 위해 인코딩된 비디오 파일에 첨부된 접미사입니다.

* **[!UICONTROL 비디오 비트율(데이터 속도)]**  - 비디오 재생 1초(초당 킬로비트 수)를 구성하기 위해 인코딩되는 데이터의 양입니다.

* **[!UICONTROL 픽셀 너비/높이]**  - 화면 이미지의 너비 차원(픽셀 단위)입니다. 화면 이미지의 높이 차원(픽셀 단위)입니다.

* **[!UICONTROL 초당 프레임 수(fps)]**  - 각 비디오의 초당 프레임 또는 스틸 이미지 수입니다. 미국과 일본에서는 대부분의 비디오가 29.97fps로 촬영되고, 유럽 및 아시아(일본 제외)에서는 대부분의 비디오가 25fps로 촬영됩니다. 영화는 24fps로 촬영됩니다.

* **[!UICONTROL 오디오 비트 전송률]**  - 오디오 재생 1초(초당 킬로비트)를 구성하기 위해 인코딩되는 데이터의 양입니다.
...
| 8 | `4:3, 768x576, Tablet (iPad, Android™), (1200 Kbps)` | 1.5Mbps | _iPad_768x576_1200K | 1200년 | 768x576 | 소스와 동일 | 96 | 고해상도, WiFi |
| 9 | `16:9, 1280x720, Desktop, (2000 Kbps)` | 3.0Mbps | _1280x720_2000K | 2000 | 1280x720 | 소스와 동일 | 128년 | 고화질, 와이드스크린 |
| 10 | `4:3, 1280x960, Desktop, (2000 Kbps)` | 3.0Mbps | _1280x960_2000K | 2000Kbps | 1280x960 | 소스와 동일 | 128년 | 고화질 |
...
Dynamic Media Classic HTML5 비디오 뷰어를 사용하는 것이 좋습니다. HTML5 비디오 뷰어에 사용된 사전 설정은 강력한 비디오 플레이어입니다.
...
리치 미디어 콘텐츠의 범위를 데스크탑, 태블릿 및 모바일 사용자로 확장하고 간소화된 비디오 경험을 제공할 수 있습니다.
...
* **[!UICONTROL eCatalog Viewer]**  - 인쇄된 카탈로그를 읽는 경험을 시뮬레이션합니다. 페이지에서 페이지로 이동하고, 페이지에서 항목을 확대 및 축소하거나, 이미지 맵을 사용하여 페이지의 항목에 대한 자세한 정보를 보거나, 카탈로그를 검색할 수 있습니다. 맵 영역에 유효한 rollover_key 특성이 있는 경우 정보 패널을 포함하여 자세한 정보와 이미지 매핑된 항목을 표시할 수도 있습니다. 정보 패널을 포함하려면 [eCatalog 뷰어 사전 설정] 창의 [정보 패널 설정] 패널에서 정보 서버 URL을 지정합니다.

* **[!UICONTROL 견본 집합 뷰어]**  - 이미지를 다른 색상, 재료, 텍스처, 마무리 또는 원단으로 표시합니다. 축소판을 선택하여 이미지의 변형을 확인합니다.

* **[!UICONTROL 혼합 미디어 세트 뷰어]**  - 한 뷰어에 다른 유형의 미디어를 표시합니다. 견본 집합, 회전 집합, 이미지 및 비디오를 포함할 수 있습니다. 이미지 집합용 탭, 비디오용 탭 등 각기 다른 컨텐츠 유형을 포함할 탭을 설정할 수 있습니다. 혼합 미디어 집합에서 재생된 비디오는 타임라인과 [중지], [일시 중단], [되감기], [재생] 등의 비디오 컨트롤이 포함된 표준 비디오 뷰어를 사용합니다. 혼합 미디어 집합 뷰어 사전 설정을 지정할 때 혼합 미디어 집합의 각 자산 유형에 사용하려는 뷰어를 지정합니다. [그리드 뷰어] 또는 [회전식 뷰어]를 사용하여 혼합 미디어 집합을 볼 수도 있습니다.
...
* **[!UICONTROL 확대/축소 뷰어]**  - 사용자가 영역을 선택하여 확대/축소하도록 해줍니다. 컨트롤을 선택하여 확대, 축소 및 이미지를 기본 크기로 재설정할 수 있습니다.

* **[!UICONTROL 확대/축소 뷰어: 플라이아웃]**  - 원래 이미지 옆에 확대/축소된 영역의 두 번째 이미지를 표시합니다. 사용할 컨트롤은 없습니다. 사용자가 보려는 영역으로 선택을 이동하면 됩니다.
...
[뷰어 사전 설정] 화면에서 뷰어 사전 설정을 만들고 관리합니다. 이 화면을 열려면 **[!UICONTROL 설정]** > **[!UICONTROL 뷰어 사전 설정]**&#x200B;으로 이동하십시오.

[뷰어 사전 설정] 화면에서는 다음 작업을 위한 도구를 제공합니다.

* **사전 설정 추가**  - 뷰어 사전 설정 추가  **** 대화 상자에서 추가 및 선택 을 선택합니다.
...
* **사전 설정 정렬**  - 열 제목(**[!UICONTROL 활성]**,  **[!UICONTROL 유형]**,  **[!UICONTROL 사전 설정]** 또는  **[!UICONTROL 플랫폼]**)을 선택하여 열에 있는 목록을 정렬합니다. 목록을 내림차순(또는 오름차순) 순서로 정렬하려면 두 번째 열 머리글을 선택합니다.

* **사전 설정 활성화 및 비활성화**  - 사전 설정을 선택한 다음 해당 활성 옵션을 선택하여 사전 설정을 활성화하거나 비활성화합니다.

       참조: [뷰어 사전 설정 활성화 또는 비활성화](application-setup.md#activating_or_deactivating_viewer_presets)
   
>[!NOTE]
>
>뷰어 사전 설정 페이지의 오른쪽에서 **[!UICONTROL 미리 보기]**&#x200B;를 선택하면 선택한 뷰어 사전 설정에서 자산이 어떻게 보이는지 볼 수 있습니다. 다른 자산을 보려면 뷰어 사전 설정 페이지에서 **[!UICONTROL 찾아보기]**&#x200B;를 선택하고 자산 미리 보기 선택 대화 상자에서 다른 자산을 선택합니다.
...
1. Dynamic Media Classic의 오른쪽 위 모서리 근처에 있는 **[!UICONTROL 설정]** > **[!UICONTROL 뷰어 사전 설정]**&#x200B;으로 이동합니다.
...
1. 뷰어 사전 설정 페이지의 뷰어 사전 설정 화면에서 뷰어 사전 설정을 추가하거나 편집합니다.
   ...
   * **추가**  - 도구 모음에서  **[!UICONTROL 추가]**&#x200B;를 선택합니다. 뷰어 사전 설정 추가 대화 상자에서 플랫폼을 선택하고 리치 미디어 자산 유형을 선택합니다.
          ...
          뷰어 사전 설정 만들기를 완료하면 **[!UICONTROL 다른 이름으로 저장]** 선택합니다.
...
1. 원하는 나머지 선택 사항을 설정합니다.
   ...
   >[!NOTE]
   >
   >**[!UICONTROL 소스와 같음]**&#x200B;을 선택하여 비디오 뷰어의 크기를 인코딩된 비디오 자체의 해상도 크기로 자동으로 조정합니다. 이 옵션을 선택하면 [스테이지 너비]와 [스테이지 높이]를 입력할 수 없습니다. 대신 비디오 자체에서 이 선택 사항이 제공됩니다. **[!UICONTROL 소스와 같음]**&#x200B;을 선택하는 경우 [여백 크기] 옵션을 설정하여 비디오 재생 영역 외부의 스킨 차원을 반영합니다. 이 여백 크기는 비디오 컨트롤의 픽셀 높이와 너비입니다. 다음 이미지를 사용하여 사용할 여백 크기를 결정할 수 있습니다.*
...
그러나 자체 사이트에서 뷰어 CSS 파일을 호스팅하는 경우 자체 환경의 이미지 서버에 대한 명시적 경로를 사용하여 이러한 상대적 이미지 경로를 해결해야 합니다. 예를 들어 위의 상대 경로를 명시적 경로로 업데이트하는 경우 다음과 같을 수 있습니다. 여기서 `https://s7d1.scene7.com` 은 이미지 서버의 직접 경로입니다. `https://s7d1.scene7.com/is/image/Scene7SharedAssets/FullScreenButton_dark_sprite?scl=1&fmt=png-alpha`
...
1. 선택한 자산 내보내기 대화 상자에서 **[!UICONTROL 내보내기 제출]**&#x200B;을 선택합니다.
...
자산을 표시하는 URL을 만들려면 미리 보기 대화 상자에서 사전 설정 드롭다운 목록을 열고 뷰어 사전 설정을 선택한 다음 **[!UICONTROL URL 복사]**&#x200B;를 선택합니다([뷰어 사전 설정의 URL 복사](application-setup.md#copying_the_url_of_a_viewer_preset) 참조). 이 [사전 설정] 목록은 관리자가 [뷰어 사전 설정] 화면에서 추가하고 관리하는 뷰어 사전 설정을 제공합니다. 예를 들어 모든 활성 eCatalog 뷰어 사전 설정은 사용자가 eCatalog를 미리 볼 때 [미리 보기] 대화 상자의 [사전 설정] 드롭다운 목록에 표시됩니다.
...
1. 설정 드롭다운 목록에서 **[!UICONTROL 응용 프로그램 설정]**&#x200B;을 선택합니다.
1. 설정 창의 왼쪽 창에서 **[!UICONTROL 애플리케이션 설정]** > **[!UICONTROL 뷰어]**&#x200B;로 이동합니다.
1. **[!UICONTROL 기본 뷰어]**&#x200B;를 선택합니다.
...
1. 보기에 사용할 필드를 선택합니다(모든 필드를 선택하려면 **[!UICONTROL 모두 선택]** 선택).
...
1. 다음 중 하나를 수행합니다.
   ...
   * 보기를 편집하려면 해당 보기를 선택한 다음 **[!UICONTROL 편집]**&#x200B;을 선택합니다. 그런 다음 미리 보기 패널에서 필드 이름을 선택하거나 선택 취소한 다음 **[!UICONTROL UDF 포함]** 옵션을 선택하거나 선택 취소합니다.
   * 보기를 삭제하려면 해당 보기를 선택한 다음 **[!UICONTROL 삭제]**&#x200B;를 선택합니다.
   * 뷰를 기본값으로 설정하려면 해당 뷰를 선택한 다음 **[!UICONTROL 기본값으로 설정]**&#x200B;을 선택합니다. 기본 보기는 사용자가 [세부 사항 보기]에서 자산을 열고 [메타데이터] 패널로 이동할 때 표시되는 보기입니다.

## 메타데이터 사전 설정 {#metadata-presets}

메타데이터 사전 설정을 통해 관리자는 자산에 지정되는 메타데이터를 제어 및 규정할 수 있습니다. [세부 사항 보기]에서는 사용자가 해당 목적으로 제공된 필드에 자산에 대한 메타데이터를 입력할 수 있습니다. 예를 들어 사용자는 소유자 이름, 저작권 설명 및 주소를 입력할 수 있습니다. 사용자가 이 정보를 정확하고 완전히 입력하도록 하기 위해 메타데이터 사전 설정을 만들 수 있습니다. 메타데이터 세부 사항 보기를 선택하면 메타데이터 필드에 사전 정의된 값이 채워집니다. 예를 들어 소유자 이름, 저작권 설명 및 주소가 자동으로 입력됩니다.
...
1. **[!UICONTROL 설정]** > **[!UICONTROL 애플리케이션 설정]** > **[!UICONTROL 메타데이터]** > **[!UICONTROL 메타데이터 사전 설정]**&#x200B;으로 이동합니다.
1. [메타데이터 사전 설정] 화면에서 다음 중 하나를 수행합니다.

   * 사전 설정을 만들려면 **[!UICONTROL 추가]**&#x200B;를 선택합니다. 메타데이터 템플릿 이름 텍스트 필드에 사전 설정 이름을 입력합니다. **[!UICONTROL 메타데이터 보기]**&#x200B;를 선택한 다음 드롭다운 목록에서 보기를 선택합니다( [메타데이터 보기](application-setup.md#metadata_views) 참조).
...
>[!NOTE]
>
>사용자 지정 사용자 정의 필드를 사용하여 자산을 검색하려면 **[!UICONTROL 설정]** > **[!UICONTROL 개인 설정]**&#x200B;으로 이동한 다음 검색&#x200B;]**에 UDF 포함 을 선택합니다.**[!UICONTROL  [개인 설정](personal-setup.md#personal_setup)을 참조하십시오.

### 사용자 정의 메타데이터 필드 만들기 {#creating-a-user-defined-metadata-field}

1. **[!UICONTROL 설정]** > **[!UICONTROL 애플리케이션 설정]** > **[!UICONTROL 메타데이터]** > **[!UICONTROL 사용자 정의 필드]**&#x200B;로 이동합니다.
1. **[!UICONTROL 추가]** 선택
1. [사용자 지정 필드] 대화 상자에서 원하는 선택 사항을 설정합니다.

   * **[!UICONTROL 이름]**  - 메타데이터 필드의 이름을 입력합니다.

   * **[!UICONTROL 유형]**  - 사용자가 메타데이터 필드에 입력할 수 있는 정보 유형을 정의하는 옵션을 선택합니다.

   * **[!UICONTROL 문자열]**  - 텍스트 문자열입니다.

   * **[!UICONTROL Int]**  - 정수입니다.

   * **[!UICONTROL 부동]**  - 부동 소수점 숫자입니다.

   * **[!UICONTROL 예/아니요]**  - 예/아니오 부울 값입니다.

   * **[!UICONTROL 날짜]**  - 날짜입니다. MM/DD/YYYY 형식을 사용할 수 있습니다.

   * **[!UICONTROL Filename]**  - 파일의 이름입니다.

   * **[!UICONTROL 색상]**  - 색상 이름입니다.

   * **[!UICONTROL Dimension]**  - 자산의 폭과 높이입니다.

   * **[!UICONTROL 형식화되지 않음]**  - 이전 버전과의 호환성을 위해 사용됩니다. 이 선택 사항을 선택하지 마십시오.

   * **[!UICONTROL 기본값]**  - 사용자가 필드에 입력할 가능성이 가장 큰 값을 입력합니다(선택적). 입력한 값은 만든 필드의 기본값이 됩니다.

   * **[!UICONTROL 적용 대상]**  - 선택적 메타데이터 필드를 특정 유형의 자산에만 적용하려면 자산 유형을 선택합니다.
...
1. 메타데이터 필드 만들기를 마치면 **[!UICONTROL 저장]**&#x200B;을 선택합니다.
...
* **필드 활성화**  - 필드  **** 이름 옆에 있는 작업 옵션을 선택하거나 선택 취소합니다. 회사 관리 역할을 하는 경우 이 옵션이 표시되지 않습니다. 이 옵션은 MediaPortal과 관련되어 있으므로 활성화 필드를 보려면 개인 설정에서 MediaPortal 기능 표시 를 선택(켜짐)해야 합니다.
...
회사 관리자는 모든 일괄처리 집합 사전 설정 레서피에 사용되는 기본 이름 지정 규칙을 만듭니다. 일괄 세트 사전 설정 정의에서 선택한 기본 이름 지정 규칙은 모든 웹 사이트에 대해 세트를 일괄 생성하는 데 필요한 모든 규칙일 수 있습니다. 정의한 기본 이름 지정 규칙을 사용하기 위해 일괄처리 집합 사전 설정이 만들어집니다. 회사에서 정의한 기본 이름 지정에 대한 예외가 있을 경우 특정 컨텐츠 집합에 필요한 대체 사용자 지정 이름 지정 규칙을 사용하여 배치 집합 사전 설정을 필요한 개수만큼 만들 수 있습니다. 
...
1. **[!UICONTROL 설정]** > **[!UICONTROL 애플리케이션 설정]** > **[!UICONTROL 일괄처리 집합 사전 설정]** > **[!UICONTROL 기본 이름 지정]**&#x200B;으로 이동합니다.
...
1. 필요에 따라 **[!UICONTROL 추가]**&#x200B;를 선택하여 요소에 대한 다른 이름 지정 규칙을 추가합니다. 또는 **[!UICONTROL 제거]**&#x200B;를 선택하여 요소에 대한 이름 지정 규칙을 삭제합니다.
1. **[!UICONTROL 다른 이름으로 저장]**&#x200B;을 선택하고 사전 설정의 이름을 입력합니다. 또는 기존 사전 설정을 편집하는 경우 **[!UICONTROL 저장]**&#x200B;을 선택합니다.
...
1. **[!UICONTROL 설정]** > **[!UICONTROL 응용 프로그램 설정]** > **[!UICONTROL 일괄처리 집합 사전 설정]** > **[!UICONTROL 일괄처리 집합 사전 설정]**&#x200B;으로 이동합니다. [세부 사항] 페이지의 오른쪽 상단에서 설정되는 **[!UICONTROL 보기 양식]**&#x200B;은 기본 보기입니다.
1. 사전 설정 목록 패널에서 **[!UICONTROL 추가]**&#x200B;를 선택하여 페이지 오른쪽의 세부 정보 패널에서 정의 필드를 활성화합니다.
...
1. 다음 중 하나를 수행하십시오.
   ...
   * 사전 설정을 설정할 때 이름 지정 규칙을 정의하려면 **[!UICONTROL 자산 이름 지정 규칙]**&#x200B;을 확장한 다음 파일 이름 지정 드롭다운 목록에서 **[!UICONTROL 사용자 지정]**&#x200B;을 선택합니다.
...
### 2D 스핀 세트의 자동 생성을 위한 배치 세트 사전 설정을 만듭니다 {#creating-a-batch-set-preset-for-the-auto-generation-of-a-d-spin-set}
...
[일괄 처리 집합 사전 설정 만들기](application-setup.md#creating_a_batch_set_preset)를 참조하십시오.

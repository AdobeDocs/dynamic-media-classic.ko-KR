...
이미지 맵은 마우스로 롤오버하거나 클릭하여 여러 종류의 작업을 트리거할 수 있는 eCatalog 페이지의 한 영역입니다. 예를 들어 이미지 맵 위로 포인터를 이동하면 항목에 대한 롤오버-텍스트 설명이 표시됩니다. [이미지 맵]을 클릭하면 다른 작업이 시작됩니다. 예를 들어 뷰어가 항목에 대해 자세히 알아보거나 항목을 구매하고 또는 비디오를 시작하여 사용 중인 항목을 보기 위해 웹 페이지를 열 수 있습니다.
...
1. [이미지 맵] 영역에서 사각형 또는 다각형(다변형) 이미지 맵을 그립니다.
   ...
   * **다각형 맵**  - 다각형 이미지 맵 도구를 선택하고 이미지의 둘레를 필요한 만큼 클릭합니다. 클릭하면 Dynamic Media Classic이 이미지 맵의 테두리를 그립니다.
...
[템플릿을 사용하여 JavaScript™ 및 URL을 입력합니다](creating-image-maps.md#using_a_template_to_enter_javascript_and_urls).

1. (선택 사항) 표시 드롭다운 목록에서 **[!UICONTROL 롤오버 텍스트]**&#x200B;를 클릭한 다음 이미지 맵 위로 포인터를 이동할 때 화면에 표시할 텍스트를 입력합니다.
1. (선택 사항) 표시 드롭다운 목록에서 **[!UICONTROL 기타 작업]**&#x200B;을 클릭하고, 사용자가 이미지 맵 위로 포인터를 이동할 때 흐림 또는 포커스 동작을 트리거할 속성을 입력합니다.

   [이미지 맵의 다른 작업 정의](creating-image-maps.md#defining_other_actions_for_image_maps)를 참조하십시오.

1. **[!UICONTROL [저장]]**&#x200B;을 클릭합니다.
1. (선택 사항) **[!UICONTROL 미리 보기]**&#x200B;를 클릭하여 기본 eCatalog 뷰어 사전 설정으로 eCatalog 를 확인합니다.
...
고객이 이미지 맵 위로 포인터를 이동하면 도구 설명 텍스트를 선택적으로 표시할 수 있으므로 클릭 중인 내용을 알 수 있습니다.
...
* **모양 및 크기 변경**  - 사각형 이미지 맵의 크기를 조정하려면 [패닝] 도구를 선택합니다. 포인터를 경계선 또는 모서리로 이동하고 양방향 화살표 아이콘이 표시되면 드래그합니다. 다각형 이미지 맵의 크기를 조정하려면 사각형 선택 핸들을 드래그합니다. 선택 핸들을 만들려면 이미지 맵의 테두리를 클릭하고 드래그합니다.

   [이미지 맵의 위치, 모양 및 크기 조정](creating-image-maps.md#adjusting_the_position_shape_and_size_of_image_maps)을 참조하십시오.

* **이미지 맵 삭제**  - 패닝 도구를 선택하고 이미지 맵을 클릭하여 선택한 다음  **[!UICONTROL 삭제]**&#x200B;를 클릭합니다.

   eCatalog에서 모든 이미지 맵을 제거하려면 **[!UICONTROL 페이지 순서 지정]** 탭을 클릭한 다음 **[!UICONTROL 맵 지우기]**&#x200B;를 클릭합니다.
...
* **이미지 맵을 다른 페이지에 복사**  - 맵 **[!UICONTROL 에 복사]** (맵 페이지 탭에 있는지 확인)를 클릭합니다. 이미지 선택 화면에서 이미지 맵을 복사할 페이지를 선택하고 **[!UICONTROL 선택]**&#x200B;을 클릭합니다.
...
1. 롤오버 키 오류가 있는 경우 **[!UICONTROL Rollover_Key Error]** 열에서 오류를 클릭하여 스프레드시트에서 오류를 수정하려면 반드시 변경해야 하는 사항을 확인합니다. 이 메시지에서 텍스트를 선택하고 복사한 다음 스프레드시트에 붙여 넣을 수 있습니다.
1. **[!UICONTROL 미리 보기]**&#x200B;를 클릭하여 eCatalog 뷰어에서 페이지를 검사하고, X를 클릭하여 요약 화면을 닫고 맵 페이지 화면으로 돌아가거나 **[!UICONTROL 닫기]**&#x200B;를 클릭하여 찾아보기로 돌아갑니다.
...
Dynamic Media Classic에는 대상 eCatalog(이미지 맵을 복사하는 eCatalog)에 다른 크기의 페이지 또는 이미지가 다른 경우 경고 메시지가 표시됩니다. **[!UICONTROL 계속]**&#x200B;을 클릭하여 경고에도 불구하고 이미지 맵을 복사합니다.

...
1. [이미지 맵] 영역에서 사각형 또는 다각형(다변형) 이미지 맵을 그립니다.
   ...
   * **사각형 맵** - 사각형 이미지 맵 도구를 선택하고 페이지로 끌어서 사각형을 만듭니다.

   * **다각형 맵** - 다각형 이미지 맵 도구를 선택한 다음 이미지의 둘레를 필요한 만큼 선택합니다. 선택하면 Adobe Dynamic Media Classic이 이미지 맵의 테두리를 그립니다.

      이미지 맵을 그리면 Adobe Dynamic Media Classic에서 이미지 맵 목록에 이름을 지정합니다. 이름을 형성하기 위해 Adobe Dynamic Media Classic에서는 작업 중인 eCatalog 페이지의 이름에 순차적 번호를 추가합니다.
...
1. 뷰어가 이미지 맵을 선택할 때 새 웹 페이지를 열도록 할 수 있습니다. [이미지 맵] 목록 패널에서 [URL] 열에 웹 페이지의 URL을 입력합니다.
   ...
   URL(Href 템플릿)을 더 쉽게 입력하려면 을 선택합니다 **[!UICONTROL 편집]** 템플릿을 입력합니다.

자세한 내용은 [템플릿을 사용하여 JavaScript 및 URL을 입력합니다](creating-image-maps.md#using_a_template_to_enter_javascript_and_urls).

1. (선택 사항) 표시 드롭다운 목록에서 **[!UICONTROL 롤오버 텍스트]**&#x200B;을 클릭한 다음 포인터를 이미지 맵 위로 이동할 때 화면에 표시할 텍스트를 입력합니다.
1. (선택 사항) 표시 드롭다운 목록에서 **[!UICONTROL 기타 작업]**&#x200B;및 속성을 입력하여 사용자가 이미지 맵 위로 포인터를 이동할 때 흐림 또는 포커스 동작을 트리거합니다.

   자세한 내용은 [이미지 맵에 대한 다른 작업 정의](creating-image-maps.md#defining_other_actions_for_image_maps).

1. 선택 **[!UICONTROL 저장]**.
1. (선택 사항) 선택 **[!UICONTROL 미리 보기]** 기본 eCatalog 뷰어 사전 설정으로 eCatalog 를 보려면 다음을 수행하십시오.
...
1. eCatalog 이미지 맵을 그립니다.
   ...
   자세한 내용은 [전자 카탈로그 이미지 맵 그리기](creating-ecatalog-image-maps.md#drawing_ecatalog_image_maps).

1. 표시 드롭다운 목록에서 **[!UICONTROL 리치 미디어]**.
1. 왼쪽의 [자산 추가] 패널에서 임베드하려는 회전 집합 또는 비디오(MP4 형식) 자산이 들어 있는 폴더를 탐색합니다.
1. 자산을 이미지 맵으로 드래그합니다.
1. (선택 사항) 이미지 맵 목록 패널의 **[!UICONTROL 도구 설명]** 열 머리글에서 뷰어가 이미지 맵 위로 포인터를 이동할 때 화면에 표시할 텍스트를 입력합니다.
1. 선택 **[!UICONTROL 저장]**.
...
* **위치 조정** - 패닝 도구를 선택하고 포인터를 맵 테두리 위로 이동하지는 않고 가까운 곳으로 이동합니다. 포인터가 십자형 화살표로 표시되면 전체 이미지 맵을 새 위치로 드래그합니다.

   자세한 내용은 [이미지 맵의 위치, 모양 및 크기를 조정합니다](creating-image-maps.md#adjusting_the_position_shape_and_size_of_image_maps).
...
* **이미지 맵 삭제** - 패닝 도구를 선택하고 이미지 맵을 선택하여 선택한 다음 선택합니다 **[!UICONTROL 삭제]**.

   eCatalog에서 모든 이미지 맵을 제거하려면 **[!UICONTROL 페이지 순서 지정]** 탭을 선택하고 **[!UICONTROL 맵 지우기]**.

* **겹치는 이미지 맵 처리** - 드래그하여 이미지 맵 목록에서 이미지 맵 순서를 변경합니다.

   자세한 내용은 [겹치는 이미지 맵 처리](creating-image-maps.md#handling_overlapping_image_maps).

* **이미지 맵을 다른 페이지에 복사** - 선택 **[!UICONTROL 맵 복사 위치]** ( 페이지 매핑 탭에 있는지 확인합니다.) 이미지 선택 화면에서 이미지 맵을 복사할 페이지를 선택하고 을 선택합니다 **[!UICONTROL 선택]**.

   자세한 내용은 [이미지 맵을 다른 이미지에 복사](creating-image-maps.md#copying_image_maps).

>[!NOTE]
>
>이미지 맵을 eCatalog의 다른 페이지에 복사하는 것 외에도 eCatalog의 모든 이미지 맵을 다른 eCatalog에 복사할 수 있습니다. 자세한 내용은 [eCatalog 간 이미지 맵 복사](creating-ecatalog-image-maps.md#copying_image_maps_between_ecatalogs).
...
맵 요약 화면을 보려면 eCatalog 화면의 페이지 매핑 탭에서 을 선택합니다 **[!UICONTROL 요약]**.

### 이미지 맵 데이터 요약 검토 {#review-image-map-data-summary}

1. 페이지 매핑 화면에서 을 선택합니다 **[!UICONTROL 요약]**.

   [맵 요약] 화면에 eCatalog에 있는 이미지 맵, URL, 롤오버 텍스트 설명 및 기타 작업 수가 표시됩니다.

1. 롤오버 키 오류가 있는 경우 **[!UICONTROL Rollover_Key 오류]** 열에서 스프레드시트에서 변경 사항을 확인하여 오류를 수정합니다. 이 메시지에서 텍스트를 선택하고 복사한 다음 스프레드시트에 붙여 넣을 수 있습니다.
1. 선택 **[!UICONTROL 미리 보기]** eCatalog 뷰어에서 페이지를 검사할 수 있습니다. X 를 선택하여 요약 화면을 닫고 맵 페이지 화면으로 돌아가거나 를 선택합니다 **[!UICONTROL 닫기]** 찾아보기 로 돌아갑니다.
...
1. 메타데이터 가져오기 대화 상자에서 다음을 선택합니다 **[!UICONTROL 찾아보기]**&#x200B;그런 다음 탭으로 구분된 파일 또는 XML DTD 파일을 선택합니다.
...
1. 복사할 이미지 맵이 있는 eCatalog를 선택하고 eCatalog의 롤오버를 선택합니다 **[!UICONTROL 편집]** 버튼을 클릭합니다.
1. 주문 페이지 탭에서 을 선택합니다 **[!UICONTROL 맵 복사]**.
1. 자산 선택 대화 상자에서 이미지 맵을 복사할 eCatalog를 선택한 다음 선택합니다 **[!UICONTROL 선택]**.

Adobe Dynamic Media Classic은 대상 eCatalog(이미지 맵을 복사하는 eCatalog)에 다른 크기의 페이지 또는 이미지가 다른 경우 경고 메시지를 표시합니다. 선택 **[!UICONTROL 계속]** 경고 메시지가 표시되더라도 이미지 맵을 복사하기 위해

...
1. (선택 사항) 표시 드롭다운 목록에서 **롤오버 텍스트**&#x200B;를 클릭한 다음 포인터를 이미지 맵 위로 이동할 때 화면에 표시할 텍스트를 입력합니다.
1. (선택 사항) 표시 드롭다운 목록에서 **기타 작업**&#x200B;을 클릭하고 포인터를 이미지 맵 위로 이동할 때 흐림 또는 초점 동작을 트리거할 속성을 입력합니다.

   [이미지 맵의 다른 작업 정의](creating-image-maps.md#defining_other_actions_for_image_maps)를 참조하십시오.

1. **[저장]**&#x200B;을 클릭합니다.
1. (선택 사항) 기본 eCatalog 뷰어 사전 설정으로 eCatalog를 보려면 **미리 보기**&#x200B;를 클릭합니다.
...
* **이미지**
맵 삭제팬 도구를 선택하고 이미지 맵을 클릭하여 선택한 다음 
**삭제**.

   eCatalog에서 모든 이미지 맵을 제거하려면 **페이지 주문** 탭을 클릭한 다음 **맵 지우기**&#x200B;를 클릭합니다.
...
* **이미지 맵을 다른**
페이지에 복사매핑 대상 복사 단추를 선택합니다([맵 페이지] 탭에 있는지 확인). [이미지 선택] 화면에서 이미지 맵을 복사할 페이지를 선택하고 [선택] 단추를 클릭합니다. 
...
Dynamic Media Classic에서는 대상 eCatalog(이미지 맵을 복사하는 eCatalog)의 페이지 수 또는 이미지 크기가 다른 경우 경고 메시지를 표시합니다. [계속]을 클릭하여 경고를 무시하고 이미지 맵을 복사할 수 있습니다.

...
이러한 옵션은 **[!UICONTROL 이미징 편집 옵션]** 제목 아래의 업로드 페이지에 있습니다.

## 이미지에서 공백 자르기

이미지에서 공백 픽셀을 자동으로 자르려면 [업로드 작업 옵션] 대화 상자에서 **[!UICONTROL 자르기 옵션]**&#x200B;을 선택합니다. **[!UICONTROL 자르기]** 드롭다운 목록에서 **[!UICONTROL 재단]**&#x200B;을 선택합니다. 다음 선택 사항을 선택합니다.

* **[!UICONTROL 기준 재단]**  - 이 드롭다운 목록에서 색상 또는 투명도를 기준으로 자르는지 여부를 선택합니다.

   * **[!UICONTROL 색상]**  - 색상 옵션 **** 을 선택합니다. 그런 다음 **[!UICONTROL 코너]** 드롭다운 목록에서 자르려는 공백 색상을 가장 잘 나타내는 색상이 있는 이미지 모서리를 선택합니다.

   * **[!UICONTROL 투명도]**  - 투명도 옵션을 선택합니다.

* **[!UICONTROL 허용치]**  - 슬라이더를 드래그하여 0에서 1까지의 허용치를 지정합니다.

   * **색상을 기반으로 트리밍**  - 이미지 모서리에서 선택한 색상과 정확히 일치하는 경우에만 픽셀을 자르도록 0을 지정합니다. 값이 1에 가까워질수록 색상 차이를 더 많이 허용합니다.
...
클리핑 패스 정보를 기반으로 이미지의 마스크를 만들려면 **[!UICONTROL 클리핑 패스에서 마스크 만들기]**&#x200B;를 선택합니다. 이 선택 사항은 클립 경로가 생성된 이미지 편집 애플리케이션으로 만들어진 이미지에 적용됩니다.
...
[이미지 선명하게 하기](sharpening-image.md#sharpening_an_image)도 참조하십시오.

[Adobe Dynamic Media 및 이미지 서버에서 이미지 선명하게 하기](/help/assets/s7_sharpening_images.pdf)도 참조하십시오.
...
| 채우기 방법 | 필수. <br>설정한 모서리 위치에서 픽셀의 투명도를 제어합니다.<br>다음 채우기 방법 중에서 선택할 수 있습니다. <br> <b>Flood 채우기</b>  - 지정한 모서리와 일치하는 모든 픽셀을 투명하게 바꿉니다.<br>• <b>픽셀 일치</b> - 이미지에서의 위치에 상관없이 모든 일치하는 픽셀을 투명하게 합니다. |

...
* **트리밍 기준**&#x200B;으로 색상 또는 투명도를 기반으로 자르기 여부를 선택합니다.
...
* **색상 기반 트리밍**&#x200B;픽셀이 이미지 모서리에서 선택한 색상과 정확히 일치하는 경우에만 픽셀을 자르려면 0을 지정합니다. 값이 1에 가까워질수록 색상 차이를 더 많이 허용합니다.

* **투명도를 기반으로 트리밍**&#x200B;픽셀이 완전히 투명한 경우에만 픽셀을 자르도록 0을 지정합니다. 1에 가깝게 있는 숫자는 더 많은 투명성을 허용합니다.
...
* **sRGB로**&#x200B;는 sRGB로 변환(표준 빨강 녹색 파랑) sRGB는 웹 페이지에 이미지를 표시할 때 권장되는 색상 공간입니다.
...
| 언샵 마스크 옵션 | 설명 |
|--- |--- |
| 금액 | 가장자리 픽셀에 적용되는 대비 양을 제어합니다.<br><br>효과의 강도로 생각하십시오. Dynamic Media Classic의 언샵 마스크 값과 Adobe Photoshop의 금액 간의 주요 차이점은 Photoshop의 양 범위는 1%에서 500%입니다. 반면에 Dynamic Media Classic에서 값 범위는 0.0에서 5.0입니다. Photoshop의 Lightroom Classic에서 5.0의 값은 500%에 상당하는 대략입니다. 0.9의 값은 90%와 같은 값입니다. |

...
>[!NOTE]
>
>Dynamic Media: Scene7 모드 사용자이고 eCatalogs를 사용하려는 경우 CRXDE Lite에서 `pdfbrochure` 값을 편집하십시오. 이렇게 하려면 Adobe Experience Manager에서 **[!UICONTROL 도구]** > **[!UICONTROL 일반]** > **[!UICONTROL CRXDE Lite]**(으)로 이동하십시오. 왼쪽 패널 탐색 트리에서 `/conf/global/settings/cloudconfigs/dmscene7/jcr:content/mimeTypes/application_pdf`(으)로 이동합니다.
>
...
eCatalog와 관련 PDF을 게시하면 웹 사이트 및 애플리케이션에 전달될 수 있도록 Dynamic Media 이미지 서버에 이 카탈로그가 배치됩니다. 게시 프로세스의 일부로 Adobe Dynamic Media Classic이 eCatalog에 대한 URL 문자열을 활성화합니다. 이 URL을 사용하여 Dynamic Media 이미지 서버에서 웹 사이트 또는 애플리케이션으로 eCatalog를 호출합니다.

eCatalog와 PDF을 [찾아보기] 패널에서 게시하도록 선택한 후 전역 탐색 막대에서 [게시] 단추를 선택하여 게시를 시작합니다. 게시 페이지에서 **[!UICONTROL 게시 제출]**&#x200B;을 선택합니다.

[전자 카탈로그 및 관련 PDF 게시](publishing-ecatalogs-associated-pdfs.md#publishing-ecatalogs-and-associated-pdfs)를 참조하십시오.

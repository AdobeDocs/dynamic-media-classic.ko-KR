...
이미지 크기 조정은 Adobe Dynamic Media Classic이 하나의 고해상도 이미지를 기반으로 여러 미분 이미지를 만드는 기능을 의미합니다. 웹 사이트나 애플리케이션용 이미지를 수동으로 여러 개 만드는 대신 하나의 1차 이미지를 제공합니다. Adobe Dynamic Media Classic은 사용자가 요청할 때 수정된 모든 이미지를 생성합니다. 단일 운영 이미지에서 이미지를 동적으로 제공하면 다음과 같은 여러 가지 이점이 있습니다.

* 이미지의 여러 사본을 다양한 크기로 수동으로 만들 필요는 없습니다. Adobe Dynamic Media Classic에 하나의 1차 이미지를 제공하면 1차 이미지에서 서로 다른 크기의 파생물이 생성됩니다.
...
다음 이미지 크기 조정 빠른 시작은 Adobe Dynamic Media Classic에서 이미지 크기 조정 기술을 사용하는 데 도움이 되도록 설계되었습니다. 1~5단계를 완료합니다. 각 단계 후에는 필요한 경우 더 많은 정보를 찾을 수 있는 상호 참조가 있습니다.

## &#x200B;1. 1차 이미지 업로드

Adobe Dynamic Media Classic에 1차 이미지를 업로드합니다. Adobe Dynamic Media Classic에서는 웹 사이트 또는 애플리케이션에서 사용할 것으로 예상되는 최대 크기인 이미지를 사용할 것을 권장합니다. 예를 들어 뷰어에서 이미지를 확대/축소하려면 가장 큰 크기가 2000픽셀 이상인 이미지를 업로드하십시오. Adobe Dynamic Media Classic은 다양한 이미지 파일 형식을 지원하지만 손실 없는 TIFF 및 PNG 이미지를 사용하는 것이 좋습니다.
...
이미지 사전 설정은 이름으로 저장된 미리 정의된 크기 및 서식 지정 명령의 컬렉션입니다. 이미지 사전 설정은 Dynamic Media 이미지 서버에서 이미지가 전달되는 크기 및 형식을 제어합니다. 회사 관리자 상태의 경우 이미지 사전 설정을 독립적으로 구성할 수 있습니다. Adobe Dynamic Media Classic에 포함된 기본 이미지 사전 설정을 사용하여 이미지를 동적으로 전달할 수 있습니다.
...
다른 이미지 사전 설정을 테스트합니다. 이미지가 다양한 크기로 웹 사이트 또는 애플리케이션에 동적으로 제공될 때 이미지가 표시되는 방식을 결정합니다.
...
* 이미지가 웹 사이트 및 애플리케이션에 동적으로 제공될 수 있도록 기본 이미지를 Dynamic Media 이미지 서버에 게시합니다.
* 게시 는 Dynamic Media 이미지 서버에서 웹 사이트 또는 애플리케이션으로 이미지를 호출하기 위한 URL 문자열을 활성화합니다. 게시 후 웹 사이트 또는 애플리케이션에서 필요한 경우 Adobe Dynamic Media Classic 생성 URL을 복사하여 배치할 수 있습니다.

전역 탐색 모음에서 **[!UICONTROL 게시]**&#x200B;를 선택하여 게시 작업을 시작합니다. 게시 대화 상자에서 **[!UICONTROL 게시 제출]**&#x200B;을 선택합니다. [기본 이미지 게시](publishing-master-images.md#publishing_master_images)를 참조하십시오.

## &#x200B;5. 웹 애플리케이션에 URL 연결

Adobe Dynamic Media Classic은 이미지에 대한 URL 콜아웃 문자열을 생성합니다. Dynamic Media 이미지 서버에 이미지를 게시하면 URL이 활성화됩니다. [찾아보기] 패널(세부 사항 보기) 또는 [미리 보기] 화면에서 이러한 URL 문자열을 복사할 수 있습니다. URL 문자열을 복사한 후에는 웹 사이트 또는 애플리케이션에서 사용할 수 있습니다. 이미지 크기 조정 URL은 웹 페이지 코드의 정적 이미지 이름에 대한 참조를 대체합니다. URL은 표시되는 각 새 이미지에 대해 데이터베이스가 대체하는 기본 이미지 이름을 참조합니다.

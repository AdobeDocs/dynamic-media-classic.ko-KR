...
응용 비디오 인코딩 사전 설정을 사용하거나 여러 개의 단일 인코딩 사전 설정을 사용하는 경우 결과는 여러 비디오 인코딩으로 자동으로 생성되는 응용 비디오 세트입니다. 개별 비디오를 선택하여 수동으로 적응형 비디오 집합을 만들 수도 있습니다.
...
1. 비디오 사전 설정 추가 페이지나 비디오 사전 설정 편집 페이지에서 원하는 비디오 사전 설정 옵션을 설정합니다.
   ...
   | 종횡비 | 종횡비는 비디오의 너비와 높이 비율입니다. 아래 나열된 처음 두 종횡비는 일반적으로 비디오를 가로로 표시하는 데 사용됩니다.<ul><li> 4:3 - 거의 모든 표준 화질 TV 방송 컨텐츠에 사용됩니다.</li><li>16:9 - 고화질 텔레비전 (HDTV)의 거의 모든 와이드 스크린 콘텐츠와 영화에 사용됩니다.</li><li>자동 크기 조정 - (기본값) 모바일, 태블릿 및 데스크톱에 전송할 비디오를 만들기 위해 모든 종횡비와 함께 작동하는 단일 인코딩 사전 설정입니다. 이 사전 설정으로 인코딩된 업로드된 소스 비디오는 고정 높이로 설정됩니다. 그러나 비디오의 종횡비(폭 대 높이 비율)를 유지하기 위해 너비가 자동으로 조절됩니다.</li><li>사용자 지정 - 표준이 아닌 비디오 크기를 정의하려 할 때 사용됩니다.</li><li>선택하는 종횡비에 따라 해상도 크기에 대한 너비 및 높이 설정이 결정됩니다. 너비 및 높이 값은 적절한 종횡비에 맞게 자동으로 조정됩니다.</li></ul> |

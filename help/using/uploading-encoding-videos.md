...
1. 비디오 사전 설정 추가 페이지나 비디오 사전 설정 편집 페이지에서 원하는 비디오 사전 설정 옵션을 설정합니다.
   ...
   | 종횡비 | 가로 세로 비율은 비디오 높이에 대한 너비의 비율입니다. 아래 나열된 처음 두 종횡비는 일반적으로 비디오를 가로로 표시하는 데 사용됩니다.<ul><li> 4:3: 거의 모든 표준 정의 TV 브로드캐스트 콘텐츠에 사용됩니다.</li><li>16:9: HDTV(High-Definition Television)의 거의 모든 와이드스크린 콘텐츠 및 영화에 사용됩니다.</li><li>자동 크기 조정: (기본값) 모바일, 태블릿 및 데스크톱에 전송할 비디오를 만들기 위해 모든 종횡비와 함께 작동하는 단일 인코딩 사전 설정입니다. 이 사전 설정으로 인코딩된 업로드된 소스 비디오는 고정 높이로 설정됩니다. 그러나 비디오의 종횡비(폭 대 높이 비율)를 유지하기 위해 너비가 자동으로 조절됩니다.</li><li>사용자 지정: 비표준 비디오 크기를 정의하려는 경우에 사용됩니다.</li><li>선택하는 종횡비에 따라 해상도 크기에 대한 너비와 높이 설정이 결정됩니다. 너비 및 높이 값은 적절한 종횡비에 맞게 자동으로 조정됩니다.</li></ul> |

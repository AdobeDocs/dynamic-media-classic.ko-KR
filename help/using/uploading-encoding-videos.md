...
* **비디오 파일 형식**: ZIP 파일과 마찬가지로 비디오 파일 형식은 파일이 비디오 파일에 포함되는 방법을 결정합니다. 일반적으로 비디오 파일에는 서로 관련되고 동기화되는 여러 트랙(오디오가 없는 비디오 트랙 하나 및 비디오가 없는 오디오 트랙 하나 이상)이 포함되어 있습니다. 비디오 파일 형식은 서로 다른 이러한 데이터 트랙과 메타데이터의 구성 방법을 결정합니다.
...
Adobe Dynamic Media Classic에 업로드하거나 Adobe Dynamic Media Classic에서 인코딩하는 모든 비디오는 &quot;비디오&quot;로 처리됩니다. 즉, 이 자산 분류는 데스크톱, 모바일 장치 또는 둘 다에서 재생하기 위해 비디오를 제공할 수 있다는 것입니다. 예를 들어 Adobe Dynamic Media Classic에서 이러한 유형의 비디오를 미리 볼 수 있습니다. 비디오 플레이어, 웹 사이트 등에서 사용하기 위해 URL 복사 기능 사용)과 임베드할 수 있는 코드(임베드된 코드 기능 사용)를 생성할 수도 있습니다.
...
1. 비디오 사전 설정 추가 페이지나 비디오 사전 설정 편집 페이지에서 원하는 비디오 사전 설정 옵션을 설정합니다.
   ...
   | 해상도 크기 | 넓은 픽셀 수와 높은 픽셀 수로 표현되는 해상도 크기가 크기를 결정합니다. 폭과 높이 값을 픽셀 단위로 입력하거나 슬라이더를 드래그하여 이러한 값을 입력합니다. 해상도 스펙트럼에는 일반적인 해상도 크기가 나열됩니다. 폭 값과 높이 값은 선택한 종횡비를 자동으로 준수합니다. 예를 들어 종횡비로 4:3을 선택하고 너비로 400을 입력하면 높이로 300이 자동으로 입력됩니다. [종횡비] 설정으로 [자동 배율]을 선택한 경우 [해상도 크기]의 [폭] 값이 자동으로 [자동]으로 설정됩니다. **[!UICONTROL 미리 보기]**&#x200B;를 선택하면 브라우저 창을 열고 해결책 선택 사항을 볼 수 있습니다. |

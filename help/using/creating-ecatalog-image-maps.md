...
1. 뷰어가 이미지 맵을 선택하면 새 웹 페이지를 열 수 있습니다. [이미지 맵] 목록 패널의 URL 열에 웹 페이지의 URL을 입력합니다.
   ...
   URL(HREF 템플릿) 입력을 단순화하려면 **[!UICONTROL 편집]**&#x200B;을(를) 선택하고 템플릿을 입력하십시오.
...
eCatalog의 리치 미디어 옵션을 사용하여 eCatalog에 추가한 이미지 맵에 MP4 형식의 비디오나 회전 집합을 추가할 수 있습니다. 사용자가 eCatalog에서 이미지 맵 영역을 선택하면 관련 회전 집합 또는 비디오가 표시됩니다. 이 기능은 고객이 사용 중에 항목을 보거나 다른 각도에서 항목을 보기를 원하는 경우에 특히 유용합니다.
...
* **위치 조정**: 패닝 도구를 선택하고 맵 위에 포인터를 놓지 않고 맵의 테두리 근처에서 포인터를 이동합니다. 포인터에 방향 화살표 아이콘이 표시되면 전체 이미지 맵 을 새 위치로 드래그합니다.
...
>[!NOTE]
>
>eCatalog의 다른 페이지에 이미지 맵을 복사할 때 한 eCatalog의 모든 이미지 맵을 다른 eCatalog에 복사할 수도 있습니다. [전자 카탈로그 간 이미지 맵 복사](creating-ecatalog-image-maps.md#copying_image_maps_between_ecatalogs)를 참조하십시오.

## 이미지 맵 데이터 검토 및 가져오기 {#reviewing-and-importing-image-map-data}

[맵 요약] 화면에서는 eCatalog에 대한 메타데이터를 제공합니다. [맵 요약] 화면에서 시작하여 eCatalog의 이미지 맵 데이터를 일괄 가져올 수도 있습니다. 이러한 방식으로 이미지 맵 데이터를 가져오면 이미지 맵 URL 및 롤오버 텍스트 입력이 간단해집니다.
...
1. 롤오버 오류가 있는 경우 **[!UICONTROL 롤오버 키 오류]** 열에서 오류를 선택하여 이를 수정하기 위해 스프레드시트에서 업데이트해야 하는 내용을 확인합니다. 이 메시지에서 텍스트를 선택하고 복사한 다음 스프레드시트에 붙여 넣을 수 있습니다.
...
각 페이지에 이미지 맵 데이터를 입력하는 대신 전체 eCatalog의 데이터를 맵 요약 화면에 가져올 수 있습니다. 탭으로 구분된 파일이나 XML DTD 형태로 이미지 맵 데이터를 가져옵니다. 파일의 필드는 맵 요약 화면에 표시되는 순서(이름, 목차 레이블, 맵, URL, 롤오버 텍스트, 기타 작업 및 검색 문자열)를 따라야 합니다. 이미지 맵 데이터를 가져오면 각 이미지 맵을 만들 때 이미지 맵 목록에 데이터를 입력할 필요가 없습니다.
...
1. 작업 이름 필드에 파일 이름을 입력합니다(확장자를 유지해야 함).
1. **[!UICONTROL 업로드]**&#x200B;를 선택합니다.

## 전자 카탈로그 간 이미지 맵 복사 {#copying-image-maps-between-ecatalogs}

eCatalog에 있는 모든 이미지 맵을 다른 eCatalog로 복사할 수 있습니다. 이 방법으로 이미지 맵을 복사하면 동일한 eCatalog의 현지화된 버전 간에 이미지 맵을 복사할 수 있습니다. 복사가 성공하려면 Adobe Dynamic Media Classic에서는 페이지 수가 같고 이미지 크기가 동일한 eCatalogs 간에 복사하는 것이 좋습니다.
...
이미지 맵을 복사하는 대상 eCatalog에 크기가 다른 페이지 또는 이미지 수가 다른 경우 Adobe Dynamic Media Classic에 경고 메시지가 표시됩니다. 경고에도 불구하고 이미지 맵을 복사하려면 **[!UICONTROL 계속]**&#x200B;을(를) 선택하십시오.

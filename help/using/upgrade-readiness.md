...
|  | 작업 | 설명 |
| :--- | :--- | --- |
| **1단계: 라이선스** | 계약 실행 | Adobe 계정 팀은 트래픽 및 스토리지를 기반으로 사용자와 함께 [!DNL Dynamic Media Classic] 라이선스에서 [!DNL Dynamic Media] 라이선스에서 갱신할 수 있도록 전환합니다. |
| **2단계: 준비** | 기능 사용 확인 | [!DNL Dynamic Media Classic]에서 사용 중인 기능을 [!DNL Dynamic Media]에서 사용할 수 있는지 확인하십시오. [기능 비교](/help/using/upgrade-feature-comparison.md) 페이지를 참조하세요. [!DNL Dynamic Media]에서 아직 사용할 수 없는 주요 기능은 다음과 같습니다.<br>· Visual Configurator(이미지 작성자, 이미지 렌더링).<br>· 이미지 템플릿(1:1 템플릿).<br>· eCatalogs.<br>위의 기능을 사용 중인 경우 [!DNL Dynamic Media Classic]을 통해 해당 기능에 액세스할 수 있다는 가정하에 업그레이드를 수행할 수 있습니다. |

...
1. 템플릿을 저장할 폴더를 선택하고 템플릿 이름을 입력한 다음 **[!UICONTROL 제출]**&#x200B;을 선택합니다.
   ...
   Adobe Dynamic Media Classic에서는 필요한 경우 템플릿 정의를 위한 템플릿 화면의 영역인 캔버스에 맞게 이미지의 크기를 조정합니다.

## 템플릿 집합 편집 {#editing-a-template-set}

게시된 집합을 편집하든 게시되지 않은 템플릿 집합을 편집하든 **[!UICONTROL 저장 후 게시]** 옵션은 다음과 같은 방법으로 집합 및 집합 구성원에 영향을 줍니다.
...
템플릿 세트를 삭제하면 세트 자체가 휴지통으로 이동됩니다. 그러나 해당 세트 내의 멤버는 영향을 받지 않습니다. 대신 각 멤버가 기존의 게시된 상태 또는 게시되지 않은 상태를 유지합니다.
...
* **[!UICONTROL 텍스트]**: 텍스트 레이어를 만듭니다. 텍스트 레이어를 만들려면 캔버스에서 드래그한 다음 레이어에 텍스트를 입력합니다. [텍스트 레이어 만들기](#creating-a-text-layer)를 참조하십시오.
...
* **[!UICONTROL 텍스트 편집기 v4.3 및 텍스트 편집기 v4.2]**: 가장 최신의 모든 기능을 갖춘 텍스트 편집기를 사용하십시오. 텍스트 편집기 v4.3 또는 이전 텍스트 편집기 v4.2를 사용하도록 선택할 수 있습니다. 템플릿을 만들 때 기본적으로 텍스트 편집기 v4.3이 선택됩니다. 이전 템플릿을 편집하면 기본적으로 텍스트 편집기 v4.2가 선택됩니다. 텍스트 편집기 v4.3은 현재 자동 줄 바꿈을 지원하지 않으므로 자동 줄 바꿈을 사용하는 이전 템플릿을 편집할 때는 텍스트 편집기 v4.2를 사용하여 템플릿의 충실도를 그대로 유지합니다. 이전 템플릿에서 자동 줄바꿈을 사용하지 않는 경우 텍스트 편집기 v4.3을 선택하여 제공되는 여러 가지 새로운 기능을 활용할 수 있습니다. 예를 들어 여백 증가, 여백 감소, 텍스트를 모두 대문자로 설정, 맞춤 텍스트 복사 등이 있습니다.
...
1. 캔버스 또는 이미지에 텍스트 상자를 만들려면 드래그합니다.
1. 텍스트 화면이 열리면 미리보기 탭에서 다음 중 하나를 수행하여 텍스트를 추가합니다.

   * 텍스트 상자에 텍스트를 입력합니다. 텍스트 상자에 텍스트를 맞추려면 [텍스트 맞춤]을 선택합니다.
...
1. 다음 서식 옵션을 지정한 다음 **[!UICONTROL 적용]**&#x200B;을 선택합니다.
   ...
   * **[!UICONTROL 굵게]**: 텍스트를 선택한 다음 텍스트에 적용할 서식 유형의 아이콘을 선택합니다.

   * **[!UICONTROL 기울임꼴]**: 텍스트를 선택한 다음 텍스트에 적용할 서식 유형의 아이콘을 선택합니다.

   * **[!UICONTROL 밑줄]**: 텍스트를 선택한 다음 텍스트에 적용할 서식 유형의 아이콘을 선택합니다.

   * **[!UICONTROL 모두 대문자]**: 텍스트를 선택한 다음 텍스트에 적용할 서식 유형의 아이콘을 선택합니다.

   * **[!UICONTROL 위 첨자]**: 텍스트를 선택한 다음 텍스트에 적용할 서식 유형의 아이콘을 선택합니다.

   * **[!UICONTROL 아래 첨자]**: 텍스트를 선택한 다음 텍스트에 적용할 서식 유형의 아이콘을 선택합니다.
...
>[!NOTE]
>
>마지막 작업을 취소하려면 **[!UICONTROL 실행 취소]**&#x200B;를 선택하십시오. **[!UICONTROL 실행 취소]**&#x200B;를 선택한 후 작업을 취소하려면 **[!UICONTROL 다시 실행]**&#x200B;을 선택하십시오.
...
특히 투명도나 중복 인쇄가 포함된 경우 레이어 순서를 변경하면 모양새에 영향을 줄 수 있습니다. 변경 사항을 저장하기 전에 결과를 미리 봅니다.

1. 템플릿의 레이어 순서를 조정하려면 다음 기술 중 하나를 사용합니다.

   * [레이어] 목록에서 레이어를 선택합니다. 그런 다음 **[!UICONTROL 위로]** 또는 **[!UICONTROL 아래로]**&#x200B;을(를) 필요한 횟수만큼 선택하여 목록에 올바로 배치하십시오.
...
* **수동으로 회전**: 커서를 레이어의 모퉁이 근처로 이동하지만 그 위로 이동하지 않습니다. 회전 커서가 표시되면 레이어 모서리를 드래그합니다. 15도 단위로 회전하려면 Shift 키를 누른 상태에서 드래그합니다.

* **도 측정 입력**: 레이어를 회전할 도 수를 입력합니다. 회전은 시계 방향이고 시계 반대 방향으로 회전하려면 음수를 입력합니다.

**레이어 또는 레이어 효과 숨기기:**

레이어 이름 또는 효과 이름 옆에 있는 눈 모양 아이콘을 선택하여 레이어 또는 레이어 효과를 숨길 수 있습니다. 숨겨진 레이어는 미리 보기나 출력에 표시되지 않습니다. 레이어 정보는 URL에서 삭제되지 않습니다. 레이어가 보이지 않도록 URL에 `hide=1`을(를) 추가합니다. 예:
...
* **불투명도**: 기본 이미지의 일부가 표시되도록 레이어를 반투명하게 하려면 [불투명도] 슬라이더를 드래그합니다. 100% 설정은 불투명하며 0은 투명합니다.

* **혼합 모드**: Adobe Photoshop에서 사용할 수 있는 혼합 모드 중 하나를 시뮬레이션하려면 옵션을 선택하십시오. 선택 사항은 [보통], [디졸브], [밝게], [어둡게], [곱하기] 및 [화면]입니다. 이러한 선택 사항은 캔버스가 아니라 레이어에 사용할 수 있습니다.

## 레이어에 그림자 및 광선 효과 사용 {#using-shadow-and-glow-effects-on-layers}

레이어에 그림자나 후광을 적용할 수 있습니다. 그림자 또는 광선은 레이어 둘레에 적용되며 선택한 그림자 또는 광선 옵션에 따라 안쪽 또는 바깥쪽으로 확장됩니다. 템플릿이 그림자 및 광선 효과가 포함된 PSD 파일에서 시작된 경우 Adobe Dynamic Media Classic에서 이러한 효과를 조정할 수 있습니다.
...
* **[!UICONTROL 불투명도]**: 효과의 강도를 확인하려면 슬라이더를 드래그합니다. 덜 불투명한 효과는 더 투명합니다.

* **[!UICONTROL 혼합 모드]**: Adobe Photoshop에서 사용할 수 있는 혼합 모드 중 하나를 시뮬레이션하려면 옵션을 선택하십시오. 선택 사항은 [보통], [디졸브], [밝게], [어둡게], [곱하기] 및 [화면]입니다.

* **[!UICONTROL 크기]**: 그림자 효과를 확대하거나 축소하려면 X 및 Y 상자에 측정값을 입력하십시오. 크기 선택 사항은 내부 그림자와 외부 그림자에만 사용할 수 있습니다.

* **[!UICONTROL 증가]**: 효과를 안쪽 또는 바깥쪽으로 확장하려면 슬라이더를 끕니다.

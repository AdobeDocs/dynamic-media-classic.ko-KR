...
* PSB 파일
* CMYK, RGB, 회색 음영 또는 비트맵 이외의 색상 공간이 있는 PSD 파일은 지원되지 않습니다. 이중톤, Lab 및 색인화된 색상 공간은 지원되지 않습니다.
...
Adobe Dynamic Media Classic은 여러 정적 파일 형식을 지원합니다. 정적 콘텐츠는 &quot;있는 그대로&quot; 게시되는 모든 에셋입니다. 예: CSS, PDF 및 XML.
...
* 기본 비디오
* PDF (기존 eCatalog/PDF 워크플로에 대한 모든 PDF를 전송하지 않도록 업로드 후 PDF을 게시하도록 표시한 경우)
* PrX 비디오
* SVG
* XML
* ZIP
...
일반적으로 에셋 파일 이름은 다음과 같이 항목 번호, 제품 SKU 또는 기타 이름과 같을 수 있습니다.
...
1. 이 작업이 완료될 때마다 이 요청을 실행할지 또는 파일이 게시될 때만 실행할지 여부를 나타냅니다.

...
* **[!UICONTROL 카탈로그 관리]**: 이 설정은 Adobe Dynamic Media Classic과 카탈로그가 상호 작용하는 방법을 결정합니다. 대부분의 웹 서버와 달리 Dynamic Media 이미지 서버 URL 호출은 적절한 이미지 파일이 아닌 매니페스트 또는 카탈로그 파일로 이동합니다. 카탈로그 파일(eCatalog와 혼동하지 않음)에는 이미지 서버에 게시된 모든 콘텐츠 목록이 포함되어 있습니다. 또한 각 이미지에 대한 경로도 포함되어 있습니다. Digimarc ID가 있는 경우 [Digimarc 사용자 정보] 섹션에서 사용자 정보를 입력합니다.
...
* **[!UICONTROL 로컬라이제이션 지원]**: 이러한 설정을 통해 여러 로케일 특성을 관리할 수 있습니다. 또한 로케일 맵 문자열을 지정할 수 있으므로 뷰어에서 여러 도구 설명을 지원하려는 언어를 정의할 수 있습니다.
  ...
  >[!NOTE]
  > 로컬라이제이션 지원 옵션을 설정하려면 [Admin Console을 사용하여 지원 사례를 만드십시오.](https://helpx.adobe.com/kr/enterprise/using/support-for-experience-cloud.html) 지원 사례에서 설치 도움말을 요청합니다.
...
>[!NOTE]
>
>로케일 맵 필드와 같은 Adobe Dynamic Media Classic에서 로컬라이제이션 지원 옵션을 설정하려면 [Admin Console을 사용하여 지원 사례를 만드십시오.](https://helpx.adobe.com/kr/enterprise/using/support-for-experience-cloud.html) 지원 사례에서 설치 도움말을 요청합니다.
...
IS는 임의의 인쇄 가능한 ASCII 문자열을 지원합니다. `locale=` 명령에는 전역 범위가 있습니다. 즉, 모든 중첩된 IS 및 IR 요청, 참조된 템플릿 및 이미지 레이어를 포함한 전체 요청에 적용됩니다. 각 레이어의 다른 로케일과 같이, 요청에 대해 여러 로케일은 지원되지 않습니다. 하지만, 중첩된 요청에서 명시적 무시의 허용은 가능할 수 있습니다.

`locale=`을(를) 지정하지 않으면 `attribute::DefaultLocale`이(가) 번역 엔진에 전달됩니다. 제한된 입력 유효성 검사가 `locale=` 값에 적용됩니다. 빈 `locale=` 값이 허용됩니다. `locale=`에 전역 범위가 있으므로 `attribute::DefaultLocale`은(는) 전체 요청에 대해 주 카탈로그에서 제공됩니다.
...
* 이미지, 이미지 세트, 비디오, 자료, 뷰어 구성 기록 등 현지화가 필요한 카탈로그에서 관리하는 모든 콘텐츠를 지원합니다.
...
알 수 없는 로케일에 대해서만 U와 같은 전용 locSuffix가 있을 수 있으며 `_U`이(가) 없는 경우 다음과 같이 기본 이미지로 강제 변환할 수 있습니다.
...
예를 들어, 서양과 중동에서 사용할 컬렉션을 지원한다고 가정합니다. 두 컬렉션은 모두 일반 이미지 컬렉션을 기반으로 하며, 모두 여러 이미지를 추가하거나 수정합니다. 그런 다음 두 컬렉션은 특정 로케일에 대해 추가로 세분화됩니다. 예를 들어, 두 개의 중동 변형의 경우 `m1, m2`이고, 세 개의 서부 로케일의 경우 `w1, w2,` 및 `w3`입니다. 단, `w1` 및 `w3`의 경우 이미지는 공유됩니다. 알 수 없는 로케일은 일반 컬렉션에만 매핑되고, 로케일별 이미지에는 액세스하지 못했습니다. 다음은 맵이 표시되는 모습입니다.
...
첫 번째 예를 기본으로 사용하여 모든 언어의 이미지에 접미사 `_1`, `_2` 또는 `_3`이(가) 있을 수 있습니다. 프랑스어 로케일과 관련된 이미지에는 접미사 `_22` 또는 `_23`이(가) 있을 수 있습니다. 독일 로케일과 관련된 이미지에는 접미사 `_470` 또는 `_480`이(가) 있을 수 있습니다.
...
* 현지화는 ID 기반 자산 호출에 제한되며, 경로 기반 자산 호출에서는 사용할 수 없습니다. 따라서, 로케일로 비디오를 호출할 경우, 회사/자산 ID로 호출해야 하며, 비디오의 전체 경로는 없습니다. `RTMP`은(는) 경로 기반 비디오 호출에만 사용되므로 현지화에는 사용할 수 없습니다.
...
* **[!UICONTROL 카탈로그 관리]**: 이 설정은 Adobe Dynamic Media Classic과 카탈로그 파일이 상호 작용하는 방법을 결정합니다. Adobe Dynamic Media Classic 렌더링 서버 URL 호출은 카탈로그에 수행되며, 이 호출은 결과적으로 서버의 이미지를 전달하기 위해 를 호출합니다. Adobe Dynamic Media Classic 지원 담당자의 도움을 받아야만 이 설정을 변경할 수 있습니다.

...
* **[!UICONTROL 카탈로그 관리]**: 이러한 설정은 Adobe Dynamic Media Classic과 카탈로그가 상호 작용하는 방법을 결정합니다. 대부분의 웹 서버와 달리 Dynamic Media 이미지 서버 URL 호출은 적절한 이미지 파일이 아닌 매니페스트 또는 카탈로그 파일로 이동합니다. 카탈로그 파일(eCatalog와 혼동하지 않음)에는 이미지 서버에 게시된 모든 콘텐츠 목록이 포함되어 있습니다. 또한 각 이미지에 대한 경로도 포함되어 있습니다. Digimarc ID가 있는 경우 [Digimarc 사용자 정보] 섹션에서 사용자 정보를 입력합니다.
...
이 명령은 대/소문자를 구분하지 않는 로케일 ID(locId) 문자열을 허용합니다. 로케일 ID는 일반적으로 문자와 &quot;`_`.&quot;
...
각 항목은 쉼표로 구분된 값 목록으로 구성됩니다. 첫 번째 값은 를 통해 전달되는 검색 값입니다 `locale=` 매개 변수. 나머지 값은 접미어/대체 값으로, 기존 이미지가 생성될 때까지 시도됩니다.
...
이 로케일을 접미사에 매핑합니다 `_E` (영어), `_G` (독일어) 및 `_F` (프랑스어). 모든 예에서 일반 입력 이미지 ID는 입니다. `myImg`.
...
예를 들어, 서양과 중동에서 사용할 컬렉션을 지원한다고 가정합니다. 두 컬렉션은 모두 일반 이미지 컬렉션을 기반으로 하며, 모두 여러 이미지를 추가하거나 수정합니다. 그런 다음 두 컬렉션은 특정 로케일에 대해 추가로 세분화됩니다. 예를 들어, `m1, m2` 2개의 중동 변형의 경우 및 `w1, w2,` 및 `w3` 3개의 서부 로케일의 경우, 단, 이미지는 `w1` 및 `w3`. 알 수 없는 로케일은 일반 컬렉션에만 매핑되고, 로케일별 이미지에는 액세스하지 못했습니다. 다음은 맵이 표시되는 모습입니다.
...
* 현지화는 ID 기반 자산 호출에 제한되며, 경로 기반 자산 호출에서는 사용할 수 없습니다. 따라서, 로케일로 비디오를 호출할 경우, 회사/자산 ID로 호출해야 하며, 비디오의 전체 경로는 없습니다. 다음을 사용할 수 없습니다. `RTMP` 를 사용하십시오. 이 메서드는 경로 기반 비디오 호출에만 사용됩니다.

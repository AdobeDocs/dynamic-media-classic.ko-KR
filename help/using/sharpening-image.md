...
[임계값]은 회색 음영 이미지의 밝기 단계 수인 0에서 255 사이의 값을 사용합니다. 0=검정, 128=50% 회색, 255=흰색입니다. 예를 들어 임계값 12는 노이즈를 추가하지 않도록 피부 톤 밝기에서 약간의 변형을 무시합니다. 반면에 대비 영역(예: 속눈썹이 피부와 접촉하는 지점)에는 여전히 가장자리 대비를 추가합니다.
...
이미지 사전 설정을 사용하지 않았거나 특정 이미지 서버 선명하게 하기 프로토콜을 URL 문자열에 전달하지 않은 경우 이미지가 다운샘플링될 때 선명하게 되지 않습니다. 그러나 선명하게 하기가 이렇게 부족한 경우에는 기본 선명하게 하기 값을 설정하여 모든 이미지에 항상 약간의 선명하게 하기가 나타나도록 할 수 있습니다.

...
AI, EPS 및 PSD 파일을 업로드할 때 [!UICONTROL 업로드 작업 옵션] 대화 상자에서 다음 작업을 수행할 수 있습니다.

* 이미지 가장자리에서 공백을 자릅니다.
* 이미지 측면에서 수동으로 자릅니다.
* 색상 프로필을 선택합니다.
* 클립 경로에서 마스크를 만듭니다.
* 언샵 마스킹 옵션을 사용하여 이미지를 선명하게 합니다.
* 배경을 녹아웃합니다.
...
이미지 측면에서 수동으로 자르려면 [자르기] 메뉴를 선택한 다음 [수동]을 선택합니다. 이미지의 임의 측면이나 각 측면에서 자를 픽셀 수를 입력합니다. 잘리는 이미지의 양은 이미지 파일의 ppi(ppi/inch) 설정에 따라 달라집니다. 예를 들어 이미지가 150ppi를 표시한다고 가정해 보겠습니다. 그런 다음 [위쪽], [오른쪽], [아래쪽] 및 [왼쪽] 텍스트 상자에 75를 입력합니다. 이 시점에서 각 면은 0.5인치 잘립니다.
...
* **[!UICONTROL 사용자 지정 변환]** > **[!UICONTROL 변환]**: 색상 공간으로 변환할 수 있도록 메뉴를 엽니다. 표준 Adobe Photoshop 색상 공간 또는 Adobe Dynamic Media Classic에 업로드한 색상 공간을 선택할 수 있습니다.
...
이 효과는 Adobe Photoshop의 언샵 마스크 필터와 동일한 옵션을 사용합니다. 이름은 헷갈릴 수 있지만 언샵 마스크는 선명하게 하기 필터입니다.
...
이미지를 업로드할 때 이미지의 배경을 자동으로 제거하려면 배경 제거 를 사용하십시오. 이 기법은 특정 오브젝트를 강조 표시하고 복잡한 배경에서 분리하는 데 유용합니다.

| 녹아웃 배경 옵션 | 설명 |
| --- | --- |
| 녹아웃 배경 | 녹아웃 배경 기능 및 옵션을 활성화하거나 &quot;켜기&quot;하려면 선택합니다. |
| 모서리 | 필수 요소입니다.<br>녹아웃할 배경색을 정의하는 데 사용되는 이미지의 모퉁이입니다.<br>왼쪽 위, 왼쪽 아래, 오른쪽 위 또는 오른쪽 아래 중 하나를 선택할 수 있습니다</b>.<b> |
| 채우기 방법 | 필수. <br>설정한 모퉁이 위치에서 픽셀 투명도를 제어합니다.<br>다음 채우기 방법 중에서 선택할 수 있습니다.<br>· <b>채우기</b>: 지정한 모퉁이와 일치하고 연결된 모든 픽셀을 투명하게 바꿉니다.<br>· <b>픽셀 일치</b>: 이미지에서의 위치에 상관없이 일치하는 모든 픽셀을 투명하게 바꿉니다. |
| 허용치 | 선택 사항입니다.<br>설정한 모퉁이 위치에 따라 픽셀 색 일치의 허용 가능한 변화 양을 제어합니다.<br>픽셀 색상을 정확하게 일치시키려면 0.0 값을 사용하십시오. 또는 1.0 값을 사용하여 가장 큰 변형을 허용합니다. |

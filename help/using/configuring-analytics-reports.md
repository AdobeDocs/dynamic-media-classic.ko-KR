...
Adobe Analytics 구성 화면을 사용하여 뷰어 이벤트를 Adobe Analytics 변수 및 Adobe Dynamic Media Classic 변수와 연결합니다. 각 뷰어 이벤트에 대해 하나의 Adobe Analytics 변수와 하나의 Adobe Dynamic Media Classic 변수를 선택합니다. Adobe Analytics 구성 화면을 여는 방법에 대한 지침은 [Adobe Analytics에 로그온](log-analytics.md#log_in_to_adobe_analytics).
...
1. Adobe Dynamic Media Classic 변수를 추가합니다.
   ...
   다음을 참조하십시오 [Adobe Dynamic Media Classic 변수](configuring-analytics-reports.md#scene7_variables).

1. Adobe Analytics 변수를 추가합니다.
1. (선택 사항) 다른 변수 쌍을 추가하려면 **[!UICONTROL 추가]**.
1. 선택 **[!UICONTROL 저장]**.

   다음을 선택한 후 **[!UICONTROL 저장]**, 뷰어 이벤트, 해당 Adobe Analytics 변수 및 해당 Adobe Dynamic Media Classic 변수가 Adobe Analytics 구성 화면에 나열됩니다.

1. 오른쪽 아래 모서리에서 을(를) 선택합니다. **[!UICONTROL 닫기]**.
1. 다음으로 이동 **[!UICONTROL 게시]** > **[!UICONTROL 게시 제출]** 이미지 제공 게시를 실행합니다.
...
Adobe Analytics 구성 화면의 각 뷰어 이벤트에 대해 Adobe Analytics 변수와 *Adobe Dynamic Media Classic 변수*. Adobe Dynamic Media Classic 변수는 보고서에 대해 가져올 수 있는 데이터를 나타냅니다. 예를 들어 `searchTerm` 변수는 eCatalog 검색에 사용되는 키워드를 나열합니다.

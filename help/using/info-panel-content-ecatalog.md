...
* [정보 패널 설정] 패널에서는 정보 패널 텍스트를 표시하는 데 사용되는 템플릿, 오류에 대한 기본 응답 및 정보가 캐시되는 시간을 지정할 수 있습니다. 또한 전자 카탈로그를 자동으로 게시할지 여부를 지정할 수 있습니다.
* 정보 패널 데이터 피드 패널을 사용하면 CSV 파일을 지정할 수 있습니다. 이 파일에는 정보 패널 롤오버 텍스트에 표시할 텍스트가 포함되어 있습니다. 정보를 업데이트할 시간을 예약할 수도 있습니다.
...
[정보] 패널에서 텍스트를 표시하기 위해 사전 설정된 세 개의 응답 템플릿 중 하나를 선택할 수 있습니다. 이러한 사전 설정된 응답 템플릿은 열 및 행 수, 서체 크기, 글꼴 및 기타 설정 등 정보 패널에 표시되는 정보의 방식을 결정합니다. 사전 설정된 응답 템플릿을 선택하거나 직접 응답 템플릿을 만들 수 있습니다.
...
1. 응답 템플릿을 선택합니다.
   ...
   * 자신만의 응답 템플릿을 만들려면 **[!UICONTROL 사용자 지정]**&#x200B;을 선택하세요. [사용자 템플릿] 상자에 템플릿 XML 정의를 입력합니다. 사전 설정 템플릿을 시작점으로 사용할 수 있습니다.

1. (선택 사항) Adobe Dynamic Media Classic에서 이미지 맵에 대한 정보를 검색하는 동안 오류가 발생할 경우 표시할 텍스트를 기본 응답 상자에 입력합니다. 예를 들어 시스템이 회사 및 eCatalog 이름은 수신하지만 롤오버 식별자는 수신하지 않은 경우 해당 사용자에게 이 메시지가 표시됩니다.
1. [응답 TTL] 상자에 데이터 캐싱 전에 대기할 시간 수를 입력합니다.

   * 하루 동안 데이터가 자주 업데이트되는 경우 더 낮은 숫자를 설정하십시오.
   * 데이터가 비교적 안정적이고 낮에 자주 업데이트하지 않아도 되는 경우 더 높은 숫자를 설정하십시오. 기본값은 10시간입니다.
...
Adobe Dynamic Media Classic에서 업로드가 성공했는지 여부를 알려주는 이메일 메시지를 보냅니다.

## 이미지 맵의 롤오버 키 텍스트 미리 보기 {#preview-rollover-key-text-for-an-image-map}

페이지 매핑 화면에서 eCatalog의 특정 페이지에 있는 이미지 맵에 대한 정보 패널 텍스트를 볼 수 있습니다.

1. 카탈로그의 롤오버 **[!UICONTROL 편집]** 단추를 선택하십시오.

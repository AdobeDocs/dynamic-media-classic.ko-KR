...
* 이미지 맵을 다른 이미지, eCatalog 페이지 또는 스핀 세트에 복사합니다.
...
1. 다음 중 하나를 수행하십시오.
   ...
   * 눈금 또는 목록 보기에서 이미지로 작업하는 경우 편집 드롭다운 목록에서 **[!UICONTROL 이미지 맵]**&#x200B;을(를) 선택합니다. 또는 세부 사항 보기에서 연 다음 이미지 위에서 **[!UICONTROL 이미지 맵]**&#x200B;을 선택합니다.
   ...
   * 그리드, 목록 또는 세부 정보 보기에서 eCatalog로 작업하는 경우 **[!UICONTROL 편집]**&#x200B;을(를) 선택합니다. **[!UICONTROL 페이지 매핑]** 탭을 선택합니다.
...
* **위치 변경**: 포인터를 이미지 맵의 테두리 근처로 이동하지 않습니다. 이동 아이콘이 표시되면 맵을 새 위치로 드래그합니다.

* **크기 및 모양 변경**: 이미지 맵의 모양과 크기를 변경하는 방법은 사각형 또는 다각형 이미지 맵으로 작업하고 있는지 여부에 따라 달라집니다.

>[!TIP]
>
>보기를 변경하고 이미지 맵을 더 명확하게 보려면 화면 하단의 크기 슬라이더를 드래그합니다.

* **사각형 이미지 맵**: 포인터를 이미지 맵의 한 쪽이나 모서리 위로 이동합니다. 양면 화살표 아이콘이 표시되면 끌기를 시작합니다. 크기를 변경하고 종횡비(모양)를 유지하려면 Shift 키를 누른 상태에서 드래그합니다.
...
각 페이지에 이미지 맵 데이터를 입력하지 않고 이미지, 스핀 세트 또는 eCatalog의 데이터를 맵 요약 화면으로 가져올 수 있습니다. 탭으로 구분된 파일이나 XML DTD 형태로 이미지 맵 데이터를 가져옵니다. 파일의 필드는 맵 요약 화면에 표시되는 순서(이름, 목차 레이블, 맵, URL, 롤오버 텍스트, 기타 작업 및 검색 문자열)를 따라야 합니다. 이미지 맵 데이터를 가져오면 각 이미지 맵을 만들 때 이미지 맵 목록에 데이터를 입력할 필요가 없습니다.
...
1. (선택 사항) **[!UICONTROL 생성]**&#x200B;을 선택합니다. 만들려는 파일 형식을 기반으로 결과 데이터를 미리 봅니다. 메타데이터 업로드 대화 상자로 돌아가려면 **[!UICONTROL 닫기]**&#x200B;를 선택합니다.
...
이미지나 eCatalog 페이지의 이미지 맵을 다른 이미지나 eCatalog 페이지에 복사할 수 있습니다. 만들기 프로세스를 단순화하려면 **[!UICONTROL 이미지 맵 복사]**&#x200B;를 사용하십시오. 레이아웃이나 매핑 구조를 공유하는 이미지 또는 페이지에서 이미지 맵을 만들려면 이미지 맵을 복사할 수도 있습니다.

예를 들어 eCatalog에서 이미지 맵을 복사하면 동일한 eCatalog의 서로 다른 언어 버전 간에 모든 이미지 맵을 편리하게 복사할 수 있습니다. 최상의 결과를 얻으려면 페이지 수가 동일한 eCatalogs와 이미지가 동일한 간에 복사하는 것이 가장 좋습니다. 복사하는 eCatalog에 이미 이미지 맵이 포함되어 있는 경우 복사가 수행될 때 해당 이미지 맵이 삭제됩니다.

**이미지 맵을 복사하려면:**

1. 이미지 맵 편집기 페이지(이미지 또는 회전 집합의 이미지) 또는 eCatalog 편집 화면의 [맵 페이지] 탭으로 이동합니다.
1. **[!UICONTROL 지도 복사]**&#x200B;를 선택합니다.
1. 이미지에서 이미지 맵을 복사할지 또는 eCatalog에서 이미지 맵을 복사할지 여부에 따라 다음 중 하나를 수행합니다.
...
이미지 맵 URL 입력을 단순화하기 위해 URL 템플릿(Href 템플릿이라고도 함)을 정의할 수 있습니다. 대부분의 이미지 맵 URL이 고정된 일반 형식을 공유하는 경우 URL 템플릿을 정의합니다. 고정된 URL 부분을 URL 템플릿으로 입력하면 이미지 맵을 만들 때마다 URL의 이 부분을 입력할 필요가 없습니다. URL 템플릿에는 JavaScript 명령, 경로 이름 및 매개 변수도 포함될 수 있습니다. 기본적으로 URL 템플릿에는 새 창에서 이미지를 여는 `loadProduct`이라는 고유한 Adobe Dynamic Media Classic JavaScript 처리기가 포함되어 있습니다.

>[!NOTE]
>
>JavaScript 코드를 이미지 맵의 HREF 특성에 추가하면 코드가 사용자의 컴퓨터에서 처리됩니다. 따라서 JavaScript 코드가 안전한지 확인하십시오.
...
이미지 맵 간에 변경되지 않는 모든 값은 URL 템플릿에 배치합니다. 이미지 맵 목록에서 URL 열에 변경되는 값만 추가합니다. 예:
...
* `loadProductCW`: 현재 창 내의 URL 열에 지정된 URL 대상을 표시합니다. 이 핸들러는 주로 웹 사이트 내의 페이지에 통합된 eCatalog에 사용됩니다.

* `loadProductPW`: 부모 창(현재 창을 연 페이지) 내의 URL 열에 지정된 URL 대상을 표시합니다. 현재 창은 열려 있지만 상위 창이 URL 타겟을 표시하도록 변경됩니다.
...
* **템플릿 옵션**: 개별 이미지 맵에서 URL 템플릿을 사용하지 않으려면 이미지 맵 목록에서 템플릿 옵션을 선택 취소합니다.
...
* **`onfocus`**: 이미지 맵에서 커서를 통해 포커스를 받거나 탭 또는 액세스 키를 누를 때 이벤트를 트리거합니다. 예를 들어 이미지 맵에서 포커스를 받으면 웹 페이지를 시작하고 이미지 맵에서 포커스를 잃으면 웹 페이지를 닫을 수 있습니다.

* **`onblur`**: 이미지 맵이 커서 또는 탭 이동을 통해 포커스를 잃으면 이벤트를 트리거합니다.
...
1. Adobe InDesign에서 **[!UICONTROL 창]** > **[!UICONTROL 대화형]** > **[!UICONTROL 하이퍼링크]**&#x200B;로 이동합니다.

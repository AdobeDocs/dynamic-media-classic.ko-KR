...
템플릿의 경우 다음을 선택합니다. **[!UICONTROL 추가 및 미리 보기]**&#x200B;를 클릭한 다음 선택한 매개 변수를 설정합니다. 다른 오퍼 세트 유형에는 매개 변수가 포함되어 있지 않지만 을 선택하여 사용자 정의할 수 있습니다. **[!UICONTROL 미리 보기]** 사용 가능한 사전 설정 변경
...
* **이미지 템플릿** - Adobe Dynamic Media Classic에서 이미지 템플릿을 매개 변수화할 수 있습니다. **[!UICONTROL 빌드]** > 템플릿 기본 사항 명령 매개 변수를 통해 템플릿의 구성 요소(텍스트 프레임의 텍스트, 여러 이미지)를 바꾸고 사용자 지정할 수 있습니다. 예를 들어 오퍼 집합의 경우 템플릿 매개 변수를 사용하여 오퍼 집합에 동일한 이미지의 변형을 만들 수 있습니다. 이미지 템플릿 만들기 및 매개 변수화에 대한 자세한 내용은 [템플릿 매개 변수 만들기](creating-template-parameters.md#creating_template_parameters).
...
1. 템플릿 또는 배너를 선택합니다.
1. 다음으로 이동 **[!UICONTROL 빌드]** > **[!UICONTROL 오퍼 집합 테스트 및 Target]**.

   Test&amp;Offer Set Target은 오퍼 세트의 오퍼를 나열합니다. 목록에서 첫 번째 항목은 개체입니다.
...
1. 선택 **[!UICONTROL 저장]** 또는 **[!UICONTROL 다른 이름으로 저장**]** 오퍼를 오퍼 세트의 일부로 저장합니다.
   ...
   Target Test&amp;Offer 오퍼 집합 페이지에는 생성한 오퍼가 나열됩니다.
...
Test&amp;Offer 오퍼 세트 페이지를 닫기 전에 Target 세트를 Adobe Target Standard/Premium으로 푸시합니다. 다음을 참조하십시오 [Test&amp;Offer에 Target 집합 푸시](pushing-offer-sets-target.md#pushing_offer_sets_to_target).
...
1. 오퍼 세트에 대한 이미지 또는 비디오를 조합합니다. Test&amp;Offer Target 세트 화면이나 그리드 보기 또는 목록 보기에서 시작하여 다음 방법 중 하나를 사용합니다.

   * **오퍼 집합 Target(&amp;P) 화면** - 이동 **[!UICONTROL 빌드]** > **[!UICONTROL 오퍼 집합 테스트 및 Target]**. 이미지나 비디오를 화면으로 드래그합니다. 다양한 크기의 비디오나 이미지를 만들려면 이미지나 비디오의 여러 복사본에서 끌어 각 크기를 개별적으로 설정합니다.

   * **격자 보기 또는 목록 보기** - 이미지 또는 비디오를 선택한 다음 로 이동합니다. **[!UICONTROL 빌드]** > **[!UICONTROL 오퍼 집합 테스트 및 Target]**.
...
1. 이미지 또는 비디오를 선택하고 을(를) 선택합니다(선택 사항). **[!UICONTROL 미리 보기]**. 오퍼 미리 보기 페이지에서 선택한 이미지 또는 비디오의 크기와 모양을 변경할 수 있습니다. 또는 오퍼 세트의 모든 이미지 또는 비디오를 변경할 수 있습니다.
   ...
   * 선택한 사전 설정을 오퍼 세트의 모든 오퍼에 적용하려면 다음을 선택합니다. **[!UICONTROL 모두 사전 설정 선택]** 확인란.
   ...
   선택 **[!UICONTROL 저장]** 을 클릭하여 이미지 또는 비디오 오퍼에 대한 변경 사항을 저장합니다. 그런 다음 을 선택합니다 **[!UICONTROL 닫기]** Test&amp;Offer Target 세트 페이지로 돌아갑니다.
...
Test&amp;Offer Target 세트 페이지를 닫기 전에 오퍼 세트를 Adobe Target Standard/Premium으로 푸시합니다. 다음을 참조하십시오 [Test&amp;Offer에 Target 집합 푸시](pushing-offer-sets-target.md#pushing_offer_sets_to_target).
...
1. Test&amp;Offer Set Target에서 다음 중 하나를 수행합니다.

   * **오퍼 제거** - 오퍼를 선택한 다음 을 선택합니다. **[!UICONTROL 삭제]** 집합에서 오퍼를 제거합니다.
   * **오퍼 추가** - 오퍼를 추가하는 방법은 작업 중인 오퍼 세트의 유형에 따라 다릅니다.
      * **템플릿** - 선택 **[!UICONTROL 추가 및 미리 보기]**&#x200B;을 클릭하고 오퍼 추가 및 미리 보기 페이지에서 다른 오퍼를 만듭니다.
      * **이미지 및 비디오** - 이미지 또는 비디오를 Test&amp;Offer Target 세트 페이지로 드래그합니다.

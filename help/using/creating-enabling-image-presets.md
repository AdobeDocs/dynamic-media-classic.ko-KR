...
Media Portal 관리자는 이미지 사전 설정을 만들어 이미지를 내보낼 때 형식을 다시 지정하는 방법을 제어할 수 있습니다. 이미지 사전 설정은 사용자가 Adobe Dynamic Media Classic에서 이미지를 내보낼 때 회사 사양으로 이미지 형식을 변경합니다. 사용자는 수동으로 이미지의 형식을 다시 지정하는 대신 이미지 사전 설정의 정확한 사양에 맞게 이미지를 내보냅니다.

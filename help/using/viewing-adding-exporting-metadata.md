...
* 사용자 정의 필드의 이름은에서 생성된 것으로 식별됩니다. **[!UICONTROL 설정]** > **[!UICONTROL 응용 프로그램 설정]** > **[!UICONTROL 메타데이터]** > **[!UICONTROL 사용자 정의 필드]**. 사용 `Generate file` 정의된 모든 UDF 목록을 올바른 가져오기 형식으로 가져오는 기능.
...
* 메타데이터 스키마 속성은 속성 이름 앞에 관련 접두어가 있어야 합니다. 접두어와 이름은 콜론으로 구별합니다. 접두사 및 속성 이름은 메타데이터 스키마 편집기에서 정의됩니다. 메타데이터 스키마 속성 이름이 `Generate file` 기능.
...
여러 파일에 대한 메타데이터를 가져올 수 있습니다. 탭으로 구분된 파일 또는 XML 파일에 메타데이터를 입력합니다. 그런 다음 을 선택합니다. **[!UICONTROL 메타데이터 파일 처리]** 업로드 작업 옵션(FTP 탭을 통해) 페이지에서 다음을 수행합니다.
...
전역 탐색 모음에서 를 선택합니다. **[!UICONTROL 업로드]**. 메타데이터를 가져오려면 업로드 페이지에서 다음을 선택합니다. **[!UICONTROL FTP를 통해]** 탭을 선택한 다음 를 선택합니다 **[!UICONTROL 작업 옵션]**. 업로드 작업 옵션 대화 상자에서 다음을 선택합니다. **[!UICONTROL 작업]**&#x200B;을(를) 선택한 다음 **[!UICONTROL 메타데이터 파일 처리]** 확인란.

...
1. 왼쪽의 에셋 라이브러리 패널에서 에셋 폴더로 이동합니다. 여기에는 복사하려는 포함 코드가 있는 미디어 세트가 포함됩니다.
...
혼합 미디어 세트를 배포하는 표준 방법은 웹 페이지에 링크(탐색 아이콘 사용)를 추가하는 것입니다. 선택하면 링크가 혼합 미디어 집합 뷰어에 혼합 미디어 집합을 표시하는 동적 페이지(ASP 또는 JSP)를 시작합니다.

## 혼합 미디어 집합 뷰어의 포함된 코드 복사 {#copying-the-embed-code-of-a-mixed-media-set-viewer}

포함된 코드 기능을 사용하면 선택한 혼합 미디어 세트에 대한 뷰어 코드를 검토할 수 있습니다. 또한 코드를 클립보드에 복사하여 웹 페이지에 붙여 넣어 뷰어를 배포할 수 있습니다. 코드 편집은 [임베드 코드] 대화 상자에서 허용되지 않습니다.

**혼합 미디어 집합 뷰어의 포함된 코드를 복사하려면:**

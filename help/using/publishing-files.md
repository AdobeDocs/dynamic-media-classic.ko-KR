...
일부 종속 자산은 상위 자산이 게시로 표시되면 자동으로 게시로 표시됩니다. 이 표에는 자동으로 게시되도록 표시된 하위 자산이 나열됩니다.
...
파생 이미지는 상위 이미지가 게시될 때 자동으로 게시되도록 표시됩니다. 파생 이미지에는 이미지 편집 선택 사항으로 조정한 이미지도 포함됩니다. 기본 및 파생 아래의 세부 사항 보기에서 이러한 파생된 이미지를 볼 수 있습니다.

## 게시 작업 만들기 {#creating-a-publish-job}

Adobe Dynamic Media Classic 서버에 업로드한 자산을 게시하는 게시 작업을 만듭니다. 자동으로 게시하지 않으려는 자산에 사용합니다. 1회 게시 작업을 수행하거나 작업이 정기적으로 반복되도록 예약할 수 있습니다. Adobe Dynamic Media Classic은 특정 서버에 게시하기 위한 고급 게시 옵션과 이미 게시된 에셋을 다시 게시하기 위한 옵션을 제공합니다.
...
그런 다음 **[!UICONTROL 일별]**, **[!UICONTROL 주별]**, **[!UICONTROL 월별]** 또는 **[!UICONTROL 사용자 지정]**&#x200B;의 반복 옵션을 선택한 다음 게시 작업을 다시 실행할 시기를 지정하십시오. Adobe Dynamic Media Classic은 반복 게시 작업을 예약하기 위한 캘린더 도구를 제공합니다. **[!UICONTROL 사용자 지정]** 옵션을 선택하고 규칙 텍스트 필드에 규칙을 입력하여 사용자 지정 작업 간격을 설명할 수 있습니다.

[사용자 지정 업로드 또는 게시 작업 시간 간격 만들기](checking-job-files.md#creating_a_custom_upload_or_publish_job_time_interval)를 참조하십시오.

>[!NOTE]
>
>작업 페이지에 반복 게시(및 업로드) 작업이 나열됩니다. 작업 페이지의 [일정이 잡힘] 탭으로 이동하여 예약된 작업을 편집하거나 삭제합니다.
...
* **[!UICONTROL 다음으로 작업 실행]**: 목록에서 사용자 이름을 선택합니다. 작업 페이지에서 사용자 이름을 기준으로 작업을 정렬할 수 있습니다. 이름을 선택하여 게시 작업을 사용자와 연결합니다.
...
>[!NOTE]
>
>게시 작업을 취소하면 작업이 안전하게 중지될 수 있는 지점에 도달할 때까지 상태가 &quot;중지 중&quot;으로 변경됩니다. 작업이 데이터베이스에서 데이터를 가져오는 진행 중인 경우 게시 작업을 중지하는 데 시간이 걸릴 수 있습니다.

## 자산 수동 게시 {#manually-publishing-assets}

게시 작업을 만드는 대신 개별 자산을 수동으로 게시할 수 있습니다. 세트를 게시하면 해당 세트 내의 상위 및 모든 하위 세트가 게시됩니다.

자산 이름 왼쪽에 있는 줄(게시되지 않은 상태)이 있는 회색의 원형 아이콘은 사용자 인터페이스에 게시되지 않은 자산을 나타냅니다. 자산이 게시되면 이 아이콘은 녹색으로 변하고 중앙에 흰색 확인 표시(게시된 상태)가 생깁니다.
...
1. 다음 중 하나를 수행하십시오.
   ...
   * 그리드 보기, 목록 보기 또는 세부 사항 보기에서 에셋 이름 왼쪽에 있는 회색의 둥근 아이콘을 선택합니다.

## 수동으로 자산 게시 취소 {#manually-unpublishing-assets}

개별 자산을 수동 게시 취소할 수 있습니다. 세트를 게시 취소하면 상위 항목이 게시 취소됨 상태로 전환됩니다. 그러나 해당 집합 내의 멤버(또는 &quot;하위&quot;)는 영향을 받지 않습니다. 대신 각 멤버는 기존의 게시 또는 게시 취소 상태를 유지합니다.

에셋 이름 왼쪽에 중앙(게시된 상태)에 흰색 확인 표시가 있는 둥근 녹색 아이콘은 사용자 인터페이스에 게시된 에셋을 나타냅니다. 에셋의 게시가 취소되면 아이콘이 회색으로 바뀌고 그 사이에 줄이 표시됩니다(게시되지 않은 상태).
...
Adobe Dynamic Media Classic 자산은 CDN(content delivery network)에서 배포됩니다. CDN은 네트워크로 연결된 컴퓨터 서버 시스템입니다. 이들은 협력하여 최종 사용자에게 컨텐츠, 특히 대용량 미디어 컨텐츠를 제공합니다. CDN 시스템에서 웹 컨텐츠는 인터넷(에지 캐시 네트워크라고 함)의 웹 캐시에 저장됩니다. 웹 콘텐츠는 더 빠른 전달을 제공하기 위해 웹 캐시에서 최종 사용자에게 전달됩니다.

누군가 웹 페이지를 처음 다운로드하면 자산이 CDN 웹 캐시 서버로 전달됩니다. 이 서버는 동일한 영역의 누군가가 웹 페이지에 다음에 액세스할 때 캐시된 동일한 콘텐츠가 더 빨리 전달되도록 해당 콘텐츠를 저장합니다. 최종 사용자에 더 가까운 곳에 있기 때문에 컨텐츠가 더 빨리 제공됩니다. CDN을 사용하면 웹 페이지 표시 속도가 빨라집니다. 매번 컨텐츠가 중앙 서버에서 제공되지 않고 경계 캐시 네트워크에서 제공되므로 중앙 서버의 대역폭 요구가 감소합니다.

새로 게시된 Adobe Dynamic Media Classic 콘텐츠는 최종 사용자가 즉시 사용할 수 있으며 에지 캐시 네트워크를 빠르게 채웁니다. 그러나 새로 다시 게시된 콘텐츠(즉, 이미지 서버에 이전에 게시된 이미지와 이름이 동일한 이미지)는 CDN에서 최대 10시간 동안 업데이트되지 않습니다. 대신 최종 사용자는 CDN 네트워크의 웹 캐시에 있는 를 확인합니다. 이러한 이유로 Adobe Dynamic Media Classic에서 다시 게시한 에셋은 10시간 동안 최종 사용자에게 표시되지 않습니다.

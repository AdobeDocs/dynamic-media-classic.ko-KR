...
1. 다음 중 하나를 수행하십시오.
   ...
   * 그리드 보기 또는 목록 보기에서 이미지를 사용하여 작업하는 경우 편집 드롭다운 목록에서 를 선택합니다 **[!UICONTROL 이미지 맵]**. 또는 세부 사항 보기에서 연 다음 선택합니다 **[!UICONTROL 이미지 맵]** 이미지 위에 표시됩니다.
   * [그리드 보기] 또는 [목록 보기]에서 스핀 세트를 사용하여 작업하는 경우 **[!UICONTROL 편집]**. 또는 세부 사항 보기에서 연 다음 선택합니다 **[!UICONTROL 편집]**. 이미지 자산을 선택한 다음 을 선택합니다 **[!UICONTROL 이미지 맵]**.
   * eCatalog를 사용하여 작업하는 경우, 그리드 보기, 목록 보기, 세부 정보 보기에서 **[!UICONTROL 편집]**. 을(를) 선택합니다 **[!UICONTROL 페이지 매핑]** 탭.

   ![이미지 맵 이미지](assets/ma_image_map.png)

1. 사각형 또는 다각형(다변형) 이미지 맵을 그립니다.

   * **사각형 맵** - 사각형 이미지 맵 도구를 선택하고 페이지로 끌어서 사각형을 만듭니다. 사각형 맵에 점을 추가하려면(따라서 다각형 맵으로 변경) Ctrl 키를 누른 다음 원하는 위치에 삽입 도구를 놓고 선택합니다.
...
1. 원하는 경우 [이미지 맵] 목록에 이미지 맵 이름을 입력합니다. 이미지 맵을 그리면 Adobe Dynamic Media Classic에서 이름을 지정합니다.

   이 이름을 만들려면 Adobe Dynamic Media Classic에서 작업 중인 이미지 또는 eCatalog 페이지의 이름에 순차적 번호를 추가합니다. 선택한 이름을 입력할 수 있습니다.
...
1. 사용자가 포인터를 이미지 맵 위로 이동할 때 롤오버 텍스트를 표시하려면 [이미지 맵] 목록에 텍스트를 입력합니다. 이미지 맵 목록에서 **[!UICONTROL 표시]** 메뉴 및 선택 **[!UICONTROL 롤오버 텍스트]**. 그런 다음 사용자가 화면에 표시할 텍스트를 입력합니다. 워드 프로세서에서 텍스트를 작성하고 [롤오버 텍스트] 필드에 복사할 수 있습니다.

1. 사용자가 마우스를 이미지 맵 위로 이동할 때 다른 작업 효과를 발생시키려면 작업을 정의합니다. 에서 **[!UICONTROL 표시]** 드롭다운 목록에서 **[!UICONTROL 기타 작업]**. 작업의 특성을 입력합니다. (이동: **[!UICONTROL 표시]** > **[!UICONTROL 둘 다]** 롤오버 텍스트 및 이미지 맵에 대한 작업을 만들려면)

   자세한 내용은 [이미지 맵에 대한 다른 작업 정의](creating-image-maps.md#defining_other_actions_for_image_maps).

1. 다음 중 하나를 수행합니다(선택 사항).

   * 이미지 맵을 미리 보려면 **[!UICONTROL 미리 보기]**.
   * 이미지 맵이나 다각형 교점을 삭제하려면 이미지에서 모양을 선택한 다음 선택합니다 **[!UICONTROL 삭제]**. 또는 eCatalog의 경우, 주문 페이지 탭에서 **[!UICONTROL 맵 지우기]** 이미지 맵을 모든 페이지에서 제거하려면
...
이미지 맵의 위치, 모양 및 크기를 변경하려면 [이미지 맵] 단추 를 선택합니다. 그런 다음 **[!UICONTROL 패닝]** 도구를 사용하여 다음 지침을 따릅니다.

* **위치 변경** - 포인터를 이미지 맵의 테두리 근처로 이동하지는 않고 이동합니다. 십자형 화살표 아이콘이 표시되면 맵을 새 위치로 드래그합니다.

* **크기 및 모양 변경** - 이미지 맵의 모양과 크기를 변경하는 방법은 사각형 또는 다각형 이미지 맵으로 작업하는지 여부에 따라 달라집니다.
...
1. (선택 사항) 선택 **[!UICONTROL 생성]** 만들려는 파일 유형을 기반으로 결과 데이터를 미리 보려면 선택 **[!UICONTROL 닫기]** 을 클릭하여 메타데이터 업로드 대화 상자로 돌아갑니다.
...
이미지나 eCatalog 페이지의 이미지 맵을 다른 이미지나 eCatalog 페이지에 복사할 수 있습니다. 사용 **[!UICONTROL 이미지 맵 복사]** 빨리 만들어 내기 위해 이미지 맵을 복사하여 레이아웃이나 매핑 구조를 공유하는 이미지 또는 페이지에서 다시 만들 수도 있습니다.
...
기본적으로 URL 템플릿에는 다음과 같은 고유한 Adobe Dynamic Media Classic JavaScript 처리기가 포함되어 있습니다. `loadProduct` URL 대상이 있는 새 창이 열립니다. 그러나 JavaScript 코드를 사용하여 이 JavaScript 핸들러를 바꾸거나 다음 Adobe Dynamic Media Classic 핸들러 중 하나를 사용할 수 있습니다.
...
1. JavaScript 코드와 전체 URL(변수 부분이 달러 기호로 대체됨)을 입력합니다 [$$]). 마우스 오른쪽 단추를 클릭하고 선택하여 코드를 붙여넣을 수 있습니다 **[!UICONTROL 붙여넣기]**.
1. 선택 **[!UICONTROL 저장]**.
...
* **템플릿 옵션** - 개별 이미지 맵이 URL 템플릿을 사용하지 않도록 하려면 URL 이미지 맵 목록에서 템플릿 옵션을 선택 취소합니다.

## 이미지 맵에 대한 다른 작업 정의 {#defining-other-actions-for-image-maps}

을(를) 선택할 수 있습니다 **[!UICONTROL 표시]** 메뉴 및 선택 **[!UICONTROL 기타 작업]** 롤오버 텍스트 및 웹 페이지 실행 이외의 작업을 트리거하려면 다음을 수행하십시오. 사용자가 포인터를 이미지 맵 위로 이동할 때 작업을 시작할 수 있습니다. 이러한 작업은 World Wide Web Consortium HTML 사양에서 클라이언트측 이미지 맵에 대해 정의된 특성입니다. 그러한 보고서는 아래와 같습니다.
...
1. eCatalog 화면(eCatalog)의 맵 편집기 화면(이미지 및 스핀 세트) 또는 맵 페이지 탭에서 **[!UICONTROL 표시]** 메뉴 및 선택 **[!UICONTROL 기타 작업]**.
1. World Wide Web Consortium HTML 사양에 지정된 구문을 사용하여 [이미지 맵] 목록의 [다른 작업] 열에 지원되는 특성을 추가합니다.
1. 선택 **[!UICONTROL 저장]**.

을(를) 선택합니다 **[!UICONTROL 표시]** 메뉴 및 선택 **[!UICONTROL 둘 다]** 이미지 맵에 롤오버 텍스트 및 작업이 필요한 경우.
...
Adobe Acrobat 또는 Adobe InDesign에서 이미지 맵을 표시할 하이퍼링크 참조를 만들고 이미지 맵의 URL 위치를 지정합니다. PDF 파일을 Adobe Dynamic Media Classic에 업로드할 때 링크 추출 옵션을 선택하면 링크가 이미지 맵으로 자동으로 변환됩니다.

자세한 내용은 Adobe InDesign 도움말 또는 Adobe Acrobat 도움말을 참조하십시오.

### Adobe InDesign에서 이미지 맵 만들기 {#to-create-image-maps-in-adobe-indesign}

1. Adobe InDesign에서 **[!UICONTROL Windows®]** > **[!UICONTROL 대화형]** > **[!UICONTROL 하이퍼링크]**.
1. [하이퍼링크] 패널에서 이미지 맵에 만들 텍스트, 프레임 또는 그래픽을 선택합니다.
1. 선택 **[!UICONTROL 새 하이퍼링크]** 패널 메뉴에서 를 클릭합니다.
...
1. in **[!UICONTROL PDF 옵션]**, 선택 **[!UICONTROL 링크 추출]**.
...
1. 링크 만들기 상자에서 **[!UICONTROL 사용자 지정 링크]**, 을(를) 선택하고 을(를) 선택합니다. **[!UICONTROL 다음]**.

>[!NOTE]
>
>Adobe Acrobat에서 모양 옵션을 설정할 필요가 없습니다. Adobe Dynamic Media Classic에서 모양을 지정할 수 있습니다.

1. 링크 등록 정보 상자에서 **[!UICONTROL 작업]**.
1. 선택 **[!UICONTROL 웹 링크 열기]** 작업 선택 메뉴에서 **[!UICONTROL 추가]**.
1. URL 편집 상자에 이미지 맵의 제품 ID를 입력하고 을 선택합니다 **[!UICONTROL 확인]**. (Adobe Dynamic Media Classic은 이미지 맵 URL 템플릿을 사용하여 URL을 완료합니다.)

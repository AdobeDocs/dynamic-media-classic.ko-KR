...
관리 설정 화면은 Adobe Dynamic Media Classic 사용자를 관리하는 것입니다. 이러한 화면을 사용하여 사용자가 Adobe Dynamic Media Classic에서 작업하고 사용자와 이메일로 통신할 수 있습니다.

1. 관리 설정 옵션에 액세스하려면 다음 위치로 이동하십시오. **설정** > **개인 설정** > **관리 설정**.
...
일반적으로 Adobe Dynamic Media Classic은 첫 번째 회사 세트를 구성하고 회사 관리자를 지정합니다. 그런 다음 회사 관리자가 Adobe Dynamic Media Classic 사용자를 설정하고 관리합니다.
...
**Adobe Dynamic Media Classic 사용** 지정된 회사에 액세스할 수 있습니다. 관리 업무를 수행할 수 없습니다.

**Adobe Dynamic Media Classic 회사 관리** 자체 회사만 보고 관리할 수 있습니다. 회사 관리자는 관리자 및 사용자 추가를 비롯한 모든 관리 기능을 수행할 수도 있습니다. 회사 관리자는 DMC 회사 관리자 계정에 사용자를 추가할 수 있습니다. 이 역할이 기본 사용자 역할입니다.

사용자를 추가하면 Adobe Dynamic Media Classic에서 사용자에게 환영 이메일 메시지를 보냅니다. 메시지에는 암호 및 Adobe Dynamic Media Classic URL이 포함되어 있습니다.

### 사용자 또는 관리자 추가 {#adding-a-user-or-administrator}

1. 이동 **[!UICONTROL 설정]** > **[!UICONTROL 애플리케이션 설정]** > **[!UICONTROL 관리 설정]** > **[!UICONTROL 사용자 관리]**.
1. 선택 **[!UICONTROL 추가]**.
1. 추가할 사용자 또는 관리자의 이름과 전자 메일 주소를 입력한 다음 을 선택합니다 **[!UICONTROL 다음]**.

   >[!NOTE]
   >
   >아포스트로피 문자(`‘`)은 이메일 주소에서 사용할 수 없습니다.

1. 사용자에게 역할을 할당하려면 역할 옵션을 선택합니다.

   자세한 내용은 [Adobe Dynamic Media Classic 사용자 역할 및 권한](administration-setup.md#user_administration).

1. 사용자를 회사에 추가하려면 회사 이름을 선택합니다.
1. 사용자를 그룹에 추가하려면(Media Portal 사용자 또는 기여자를 추가하는 경우) 다음을 선택합니다 **[!UICONTROL 다음]** 사용자를 추가합니다.
1. 선택 **[!UICONTROL 저장]** 를 클릭하여 사용자 설정을 완료합니다.

   저장하면 사용자를 다른 회사에 추가할 것인지 묻는 메시지가 나타납니다. 선택 **[!UICONTROL 추가]** 사용자를 회사에 추가하려면

   새 사용자에게 무작위로 생성된 암호가 주어집니다. 사용자는 Adobe Dynamic Media Classic 데스크탑 애플리케이션에 처음 로그인할 때 암호를 변경해야 합니다.

   새 사용자를 추가하면 환영 이메일이 전송됩니다. 이 이메일은 임시 암호를 제공하고 Adobe Dynamic Media Classic에 로그인하는 방법을 설명합니다.

   사용자가 환영 이메일을 받지 못할 경우 Adobe Dynamic Media Classic 로그인 페이지(https://s7sps1.scene7.com)으로 이동하여 을 선택합니다 **[!UICONTROL 내 암호 분실]**. 암호가 재설정되고 새 이메일이 전송됩니다. 사용자가 이메일을 받지 못하고 스팸 폴더에도 없는 경우 기술 지원에 문의하십시오.

   새 Media Portal 사용자를 추가할 때 **[!UICONTROL 설정]** > **[!UICONTROL 애플리케이션 설정]** > **[!UICONTROL 사용자 관리]**&#x200B;를 선택하고 을 선택합니다. **[!UICONTROL 사용자 목록 업로드]** 500명 이하의 사용자를 포함하는 .csv 파일을 선택합니다.

### 사용자 삭제 {#deleting-a-user}

사용자를 유효하지 않게 만들어 Adobe Dynamic Media Classic에서 삭제할 수 있습니다. 무효 사용자는 시스템과 모든 계정에서 제거됩니다.

1. 이동 **[!UICONTROL 설정]** > **[!UICONTROL 애플리케이션 설정]** > **[!UICONTROL 관리 설정]** > **[!UICONTROL 사용자 관리]**.
1. 목록에서 사용자를 선택한 다음 을 선택합니다 **[!UICONTROL 편집]**.
1. [유효]를 선택 취소합니다.
1. 선택 **[!UICONTROL 저장]**.
...
1. 이동 **[!UICONTROL 설정]** > **[!UICONTROL 애플리케이션 설정]** > **[!UICONTROL 관리 설정]** > **[!UICONTROL 사용자 관리]**.
1. 사용자 목록에서 **[!UICONTROL 활성]** 옵션 을 클릭합니다.
...
1. 이동 **[!UICONTROL 설정]** > **[!UICONTROL 애플리케이션 설정]** > **[!UICONTROL 관리 설정]** > **[!UICONTROL 사용자 관리]**.
1. 목록에서 사용자를 선택한 다음 을 선택합니다 **[!UICONTROL 편집]**.
1. 사용 권한 또는 액세스를 수정할 회사를 보여 주는 테이블에서 항목을 선택한 다음 을 선택합니다 **[!UICONTROL 회사 관리]**.
1. 사용자 역할을 선택합니다.
1. 사용자 그룹 구성원을 변경하려면(Media Portal 사용자 또는 기여자를 편집하거나 추가하는 경우) **[!UICONTROL 다음]** 그룹 구성원을 편집합니다.
1. 선택 **[!UICONTROL 저장]**.
...
* **그룹별 필터링** - 을(를) 선택합니다 **[!UICONTROL 그룹별]** 메뉴 및 옵션을 선택하여 그룹의 사용자로 목록 범위를 좁힙니다.

* **사용자 역할별 필터링** - 을(를) 선택합니다 **[!UICONTROL 사용자 역할별]** 메뉴 및 옵션을 선택하여 목록 범위를 다른 유형의 사용자 또는 관리자로 좁힐 수 있습니다.

* **필드 이름별로 필터링** - 선택 **[!UICONTROL 필드별 필터 활성화]**. 그런 다음 **[!UICONTROL 필드 이름별]** 메뉴에서 목록을 필터링할 열을 선택하고 [필터 문자] 메뉴를 선택하고 문자를 선택합니다. 목록은 열 중 하나에서 선택한 문자로 필터링됩니다. 전체 목록을 보려면 **[!UICONTROL 필드별 필터 활성화]** 선택 사항입니다.

* **잘못된 사용자 필터링** - 선택 취소 **[!UICONTROL 잘못된 포함]**. 시스템에 있는 사용자만 검색 결과에 표시됩니다. 관리하는 시스템 및 계정에서 잘못된 사용자가 삭제되었습니다.

* **열 머리글별로 정렬** - 상태, 이름, 성 또는 이메일, 사용자 역할별 또는 유효/무효 상태별로 모든 사용자를 사전순으로 정렬하려면 제목을 선택합니다.
...
이 페이지를 열려면 다음 위치로 이동하십시오. **[!UICONTROL 설정]** > **[!UICONTROL 개인 설정]**. 확장 **[!UICONTROL 관리 설정]**&#x200B;를 선택한 다음 을 선택합니다. **[!UICONTROL 대역폭 및 스토리지]**.
...
* **마스크 요청** - 회색 음영 마스크를 반환하는 이미지에 대한 요청.

* **뷰어 타일 요청** - 뷰어가 로드한 이미지 요청.
...
1. 이동 **[!UICONTROL 설정]** > **[!UICONTROL 개인 설정]**.
1. 관리 설정을 확장한 다음 **[!UICONTROL 대역폭 및 스토리지]**.
1. 탭을 선택합니다. **[!UICONTROL 대역폭]**, **[!UICONTROL 스토리지]**, **[!UICONTROL 이미지 컨텐츠]**, **[!UICONTROL 도메인]**, **[!UICONTROL 비디오 스트리밍]**, 또는 **[!UICONTROL 비디오 컨텐츠]**.
...
* **차트 또는 데이터 그리드에서 데이터 보기** - 선택 **[!UICONTROL 차트 보기]** 차트에서 데이터를 보려면 선택 **[!UICONTROL 데이터 보기]** 를 눌러 데이터 그리드에서 데이터를 봅니다.

* **보고서 표시 유형 선택** - 보고서 유형 메뉴에서 **[!UICONTROL 요약]**, **[!UICONTROL 일별]**, 또는 **[!UICONTROL 월별]** 데이터를 요약 양식, 일별 또는 월별로 구성하는 방법 모든 보고서에서 이 선택 사항을 제공하는 것은 아닙니다.

* **기간 지정** - 보고서 기간을 정의하는 옵션을 선택한 다음 선택합니다 **[!UICONTROL 업데이트]** 기간을 정의한 후:

* **사전 정의된 기간** - 미리 정의된 보고서 메뉴에서 옵션을 선택합니다. 예를 들어 전달의 데이터를 캡처하려면 [지난 달]을 선택합니다.

* **사용자 지정 기간** - 미리 정의된 보고서 메뉴에서 **[!UICONTROL 사용자 지정]**. 그런 다음 **[!UICONTROL 시작 월]** 또는 **[!UICONTROL 시작 날짜]**) 메뉴 및 날짜를 월 수(또는 # 또는 일) 메뉴에 표시합니다. 도메인 및 비디오 컨텐츠 보고서의 경우 보고서 정보를 캡처할 특정 시작 날짜와 종료 날짜를 선택할 수 있습니다.

* **데이터 정렬(데이터 보기만)** - 열에 대한 정보를 정렬하려면 열 제목을 선택합니다. 내림차순으로 정렬하려면 다시 선택합니다.
...
* **보고서 데이터 내보내기** - 데이터 보기에서 필요에 따라 데이터를 정렬 및 정렬합니다. 그런 다음 를 엽니다. **[!UICONTROL 내보내기]** 메뉴를 클릭하고 형식을 선택합니다. **[!UICONTROL 탭으로 구분]**, **[!UICONTROL 쉼표로 구분됨]**, 또는 **[!UICONTROL HTML 서식]**. 선택한 형식으로 데이터가 클립보드에 복사됩니다. 이제 스프레드시트나 기타 애플리케이션에 데이터를 붙여 넣을 수 있습니다.

* **보고서 인쇄** - 선택 **[!UICONTROL 인쇄]**&#x200B;를 선택하고 인쇄 대화 상자에서 원하는 옵션을 선택한 다음 **[!UICONTROL 확인]**.
...
1. 이동 **[!UICONTROL 설정]** > **[!UICONTROL 개인 설정]**.
1. 관리 설정을 확장한 다음 **[!UICONTROL 이미지 오류]**.
1. 다음 중 하나를 수행합니다(선택 사항).
   ...
   * 링크를 실제 이미지에 복사하려면 **[!UICONTROL URL 복사 URL]**. 브라우저 창에 이 링크를 붙여 넣어 이미지로 이동하고 오류를 조사할 수 있습니다.
   * 링크를 레퍼러 웹 페이지에 복사하려면 **[!UICONTROL 레퍼러 복사 URL]**.

표시되는 오류는 현재 로그인한 회사에 대한 것입니다. 각 오류에는 다음 정보가 포함됩니다.

* **이미지 ID** - 잘못된 이미지에 대한 ID.

* **시간** - 지난 24시간 이내에 오류를 마지막으로 보고한 첫 번째 시간 범위입니다.

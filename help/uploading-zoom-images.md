...
확대/축소 이미지에는 높은 픽셀 수가 있어야 합니다. 뷰어가 이미지를 확대/축소할 때 이미지 세부 사항을 선명하게 볼 수 있도록 높은 픽셀 수가 필요합니다. 사용 중인 이미지가 Dynamic Media Classic에 업로드되기 전에 적절한 크기인지 확인하십시오.
...
* **이미지 크기** Dynamic Media Classic에서는 가장 긴 치수의 2,000픽셀 이상의 이미지 파일을 사용하는 것이 좋습니다. 이렇게 하면 사용자가 확대/축소할 때 이미지를 선명하게 볼 수 있습니다.
...
* **색상 공간** RGB는 웹 이미지 프레젠테이션을 위한 색상 공간입니다. CMYK 이미지는 업로드할 때 자동으로 RGB로 변환됩니다. RGB로 전환하는 데 사용되는 임베드된 ICC 색상 프로필이 있는 CMYK 이미지를 업로드하는 것이 좋습니다. ICC 프로필을 참조하십시오.

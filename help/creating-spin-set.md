...
일반적으로 회전 집합에 있는 이미지 수가 많을수록 이미지 회전 효과가 향상됩니다. 그러나 집합에 많은 이미지를 포함하면 이미지를 로드하는 데 걸리는 시간도 증가합니다. Dynamic Media Classic에서는 스핀 세트에 사용할 이미지를 촬영하는 데 다음 지침을 권장합니다.

* 1차원 스핀 세트의 경우 최소 8-12개의 이미지와 2차원 스핀 세트의 16-24개의 이미지를 사용합니다.
...
**삭제**.
   ...
   * **행 순서 변경(2차원 스핀 세트)**&#x200B;행 선택기 상자(행 왼쪽)를 클릭한 다음 
**행을 아래로** 이동하거나 **행을 위로**&#x200B;이동합니다.

...
Dynamic Media Classic에서 삭제한 항목은 휴지통 폴더로 이동합니다. 이러한 항목은 복원하거나 영구적으로 삭제할 때까지 이 폴더에 7일 동안 유지됩니다. 자산 라이브러리 하단에 있는 [휴지통] 아이콘 을 선택하고 휴지통 폴더에서 확인하여 삭제한 항목을 검사할 수 있습니다.
...
자산을 삭제한 사람이 복원해야 하는 것은 아니며 누구든지 휴지통 폴더에서 자산을 복원할 수 있습니다. 복원한 자산은 삭제되기 전의 폴더에 배치됩니다. 이러한 폴더가 더 이상 존재하지 않으면 Dynamic Media Classic에서 해당 폴더가 다시 만들어지고 복원된 자산이 다시 작성된 폴더에 배치됩니다.
...
* **개별 자산** 삭제 영구적으로 삭제할 자산을 선택하고 **[!UICONTROL 파일 > 휴지통에서 비만을 클릭합니다]**.

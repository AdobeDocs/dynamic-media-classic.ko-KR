...
* [메타데이터 가져오기] 대화 상자에서는 롤오버 텍스트 정보가 포함된 탭으로 구분된 TXT 파일을 가져올 수 있습니다. 롤오버 텍스트에 대해 CSV 파일 옵션과 함께 이 TXT 옵션 또는 정보 패널 데이터 피드 패널을 사용할 수 있습니다.
...
1. **[!UICONTROL 업로드]**&#x200B;를 클릭하여 rollover_key 값에 따라 정보 패널 콘텐트를 s7info에 업로드합니다.
1. S7Info 업로드 대화 상자에서 사용할 파일을 찾은 다음 **[!UICONTROL 업로드]**&#x200B;를 클릭합니다.

   지원되는 파일 형식은 UTF-16 인코딩이 포함된 탭으로 구분된 파일과 ASCII 인코딩이 포함된 CSV 파일입니다. CSV 파일의 경우 ASCII 이외의 문자는 HTML로 인코딩해야 합니다.

1. [정보 패널 설정] 패널에서 **[!UICONTROL 게시]**&#x200B;를 클릭합니다.
...
* 탭 및 쉼표로 구분된 데이터는 롤오버 템플릿에 필요한 만큼 열을 포함할 수 있습니다.
* 데이터의 첫 번째 항목이나 열은 이미지 맵 URL의 rollover_key 값과 연관된 롤오버 식별자입니다.
* 식별자 뒤에 나오는 각 탭 또는 쉼표로 구분된 항목이 응답 템플릿으로 대체하려는 항목인지 확인하십시오.따라서 첫 번째 열은 $1$로 대체되고 두 번째 열은 $2$로 대체됩니다.

### 외부에서 호스팅된 위치 {#import-csv-content-into-image-sets-from-an-externally-hosted-location}에서 CSV 컨텐츠를 이미지 세트로 가져오기

1. 이미지 집합을 두 번 클릭하여 [세부 사항 보기]에서 엽니다.
1. **[!UICONTROL 정보 패널 데이터 피드]**&#x200B;를 클릭합니다.
...
1. (선택 사항) **[!UICONTROL 업데이트 실행]**&#x200B;을 클릭하여 콘텐트를 즉시 업데이트합니다.

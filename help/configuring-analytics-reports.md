...
## Adobe Analytics 변수를 Dynamic Media Classic 뷰어 이벤트 및 변수 {#assigning-adobe-analytics-variables-to-scene-viewer-events-and-variables}에 할당
...
| PLAY | **X**(비디오) | 비디오 뷰어에서는 사용자가 [재생]을 클릭하여 비디오 재생을 시작할 때 발생합니다.<br><br>**참고: Adobe Analytics 하트비트 기반 비디오 보고를 사용하는** 경우, Dynamic Media Classic에서 Adobe Analytics을 구성할 때 이 뷰어 이벤트에 변수를 매핑할 필요가 없습니다. 비디오 하트비트는 즉시 사용 가능한 Dynamic Media Classic HTML5 비디오 및 혼합 미디어 뷰어에서 작동합니다. 비디오 플레이어는 Adobe Analytics 비디오 보고서 내에서 보기 위한 추적 데이터를 생성합니다. [Adobe Analytics 비디오 보고서 활성화](enabling-analytics-video-reports.md)를 참조하십시오. |
| PAUSE | **X**(비디오) | 비디오 뷰어에서는 사용자가 [일시 중단]을 클릭하여 비디오를 일시 중단할 때 발생합니다.<br><br>**참고: Adobe Analytics 하트비트 기반 비디오 보고를 사용하는** 경우, Dynamic Media Classic에서 Adobe Analytics을 구성할 때 이 뷰어 이벤트에 변수를 매핑할 필요가 없습니다. 비디오 하트비트는 즉시 사용 가능한 Dynamic Media Classic HTML5 비디오 및 혼합 미디어 뷰어에서 작동합니다. 비디오 플레이어는 Adobe Analytics 비디오 보고서 내에서 보기 위한 추적 데이터를 생성합니다. [Adobe Analytics 비디오 보고서 활성화](enabling-analytics-video-reports.md)를 참조하십시오. |
| STOP | **X**(비디오) | 비디오 뷰어에서는 사용자가 [중지]를 클릭하여 비디오 재생을 중지할 때 발생합니다.<br><br>**참고: Adobe Analytics 하트비트 기반 비디오 보고를 사용하는** 경우, Dynamic Media Classic에서 Adobe Analytics을 구성할 때 이 뷰어 이벤트에 변수를 매핑할 필요가 없습니다. 비디오 하트비트는 즉시 사용 가능한 Dynamic Media Classic HTML5 비디오 및 혼합 미디어 뷰어에서 작동합니다. 비디오 플레이어는 Adobe Analytics 비디오 보고서 내에서 보기 위한 추적 데이터를 생성합니다. [Adobe Analytics 비디오 보고서 활성화](enabling-analytics-video-reports.md)를 참조하십시오. |
| MILESTONE | **X**  (비디오) | 비디오 뷰어에서는 사용자가 비디오를 0, 25, 50, 75 또는 100% 시청할 때 중대한 이벤트가 생성됩니다.<br><br>**참고: Adobe Analytics 하트비트 기반 비디오 보고를 사용하는** 경우, Dynamic Media Classic에서 Adobe Analytics을 구성할 때 이 뷰어 이벤트에 변수를 매핑할 필요가 없습니다. 비디오 하트비트는 즉시 사용 가능한 Dynamic Media Classic HTML5 비디오 및 혼합 미디어 뷰어에서 작동합니다. 비디오 플레이어는 Adobe Analytics 비디오 보고서 내에서 보기 위한 추적 데이터를 생성합니다. [Adobe Analytics 비디오 보고서 활성화](enabling-analytics-video-reports.md)를 참조하십시오. |
...
Adobe Analytics 구성 화면의 각 뷰어 이벤트에 대해 Adobe Analytics 변수와 *Dynamic Media Classic 변수*&#x200B;를 선택합니다. Dynamic Media Classic 변수는 보고서에 대해 얻을 수 있는 데이터를 나타냅니다. 예를 들어 `searchTerm` 변수는 eCatalog 검색에 사용되는 키워드를 나열합니다.

다음 표에서는 Dynamic Media Classic 변수에 대해 설명합니다.

| Dynamic Media Classic 변수 | 설명 |
|--- |:--- |
| asset | Dynamic Media Classic 에셋 ID 또는 비디오 경로 파일. |
...
* **삭제**
뷰어 이벤트를 선택하고 [보기/ **[!UICONTROL 변수]** 편집] 회색 단추를 클릭합니다. **[!UICONTROL 삭제]**&#x200B;를 클릭합니다.

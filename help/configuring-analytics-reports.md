...
Adobe Analytics 보고서에 원하는 정보를 Adobe Analytics에 알려면 Adobe Analytics 구성 화면으로 이동합니다. 보고서를 구성한 후 이 화면에는 원하는 각 뷰어 이벤트에 대한 해당 Adobe Analytics 변수 및 Adobe Dynamic Media Classic 변수가 표시됩니다. 이러한 뷰어 이벤트-Adobe Analytics 변수-Adobe Dynamic Media Classic 변수 조합은 보고되는 정보를 결정합니다.
...
## Adobe Dynamic Media Classic 뷰어 이벤트 및 변수에 Adobe Analytics 변수 할당 {#assigning-adobe-analytics-variables-to-scene-viewer-events-and-variables}
...
1. Adobe Dynamic Media Classic 내에서 Adobe Analytics에 로그인하고 보고서 세트를 선택한 후, Adobe Analytics 구성 페이지의 오른쪽 테이블 열에 있는 를 선택하여 뷰어 이벤트를 활성화합니다 **[!UICONTROL 활성화]**.
...
1. Adobe Dynamic Media Classic 변수를 추가합니다.
   ...
   자세한 내용은 [Adobe Dynamic Media Classic 변수](configuring-analytics-reports.md#scene7_variables).

1. Adobe Analytics 변수를 추가합니다.
1. (선택 사항) 다른 변수 쌍을 추가하려면 를 선택합니다. **[!UICONTROL 추가]**.
1. 선택 **[!UICONTROL 저장]**.

   선택한 후 **[!UICONTROL 저장]**, 뷰어 이벤트, 해당 Adobe Analytics 변수 및 Adobe Dynamic Media Classic 변수가 Adobe Analytics 구성 화면에 나열됩니다.

1. 오른쪽 아래 모서리에서 을(를) 선택합니다. **[!UICONTROL 닫기]**.
1. 이동 **[!UICONTROL 게시]** > **[!UICONTROL 게시 제출]** 이미지 제공 게시를 실행하려면

   뷰어에 포함된 정보를 Adobe Dynamic Media Classic 서버에서 사용할 수 있도록 게시가 필요합니다.

### 뷰어 이벤트 {#viewer-events}

뷰어 이벤트는 사용자가 Adobe Dynamic Media Classic 뷰어로 수행하는 작업을 설명합니다. 사용자가 축소판 선택 또는 비디오 시작 또는 중지 등의 특정 작업을 시작하면 뷰어는 해당 이벤트와 연관된 데이터와 함께 이벤트를 웹 페이지에 &quot;브로드캐스트&quot;합니다.
...
| PLAY | **X**(비디오) | 비디오 뷰어에서 사용자가 재생 을 선택하여 비디오 재생을 시작할 때.<br><br>**참고:** Adobe Analytics 하트비트 기반 비디오 보고를 사용하는 경우, Adobe Dynamic Media Classic에서 Adobe Analytics을 구성할 때 이 뷰어 이벤트에 변수를 매핑할 필요가 없습니다. 비디오 하트비트는 기본 제공 Adobe Dynamic Media Classic HTML5 비디오 및 MixedMedia 뷰어에서 작동합니다. 비디오 플레이어는 Adobe Analytics 비디오 보고서 내에서 보기 위한 추적 데이터를 생성합니다. 자세한 내용은 [Adobe Analytics 비디오 보고서 활성화](enabling-analytics-video-reports.md). |
| PAUSE | **X**(비디오) | 비디오 뷰어에서 사용자가 선택할 때 **[!UICONTROL 일시 정지]** 비디오를 고정하려면&#x200B;<br><br>**참고:** Adobe Analytics 하트비트 기반 비디오 보고를 사용하는 경우, Adobe Dynamic Media Classic에서 Adobe Analytics을 구성할 때 이 뷰어 이벤트에 변수를 매핑할 필요가 없습니다. 비디오 하트비트는 기본 제공 Adobe Dynamic Media Classic HTML5 비디오 및 MixedMedia 뷰어에서 작동합니다. 비디오 플레이어는 Adobe Analytics 비디오 보고서 내에서 보기 위한 추적 데이터를 생성합니다. 자세한 내용은 [Adobe Analytics 비디오 보고서 활성화](enabling-analytics-video-reports.md). |
| STOP | **X**(비디오) | 비디오 뷰어에서 사용자가 선택할 때 **[!UICONTROL 정지]** 비디오 재생을 중단하려면 다음을 수행하십시오.<br><br>**참고:** Adobe Analytics 하트비트 기반 비디오 보고를 사용하는 경우, Adobe Dynamic Media Classic에서 Adobe Analytics을 구성할 때 이 뷰어 이벤트에 변수를 매핑할 필요가 없습니다. 비디오 하트비트는 기본 제공 Adobe Dynamic Media Classic HTML5 비디오 및 MixedMedia 뷰어에서 작동합니다. 비디오 플레이어는 Adobe Analytics 비디오 보고서 내에서 보기 위한 추적 데이터를 생성합니다. 자세한 내용은 [Adobe Analytics 비디오 보고서 활성화](enabling-analytics-video-reports.md). |
...
Adobe Analytics 구성 화면의 각 뷰어 이벤트에 대해 Adobe Analytics 변수와 *Adobe Dynamic Media Classic 변수*. Adobe Dynamic Media Classic 변수는 보고서에 사용할 데이터를 나타냅니다. 예를 들어 `searchTerm` 변수는 eCatalog 검색에 사용되는 키워드를 나열합니다.
...
* **활성화** - 선택 **[!UICONTROL 활성화]** 또는 **[!UICONTROL 비활성화]** 선택한 뷰어 이벤트를 비활성화하려면

* **편집** - 뷰어 이벤트를 선택하고 을(를) 선택합니다. **[!UICONTROL 보기/편집]** 변수 회색 단추. Adobe Dynamic Media Classic 변수 및 Adobe Analytics 변수 드롭다운 목록에서 각 목록에서 다른 변수를 선택합니다. 자세한 내용은 [Adobe Dynamic Media Classic 뷰어 이벤트 및 변수에 Adobe Analytics 변수 할당](#assigning-adobe-analytics-variables-to-scene-viewer-events-and-variables).

...
description: 널
seo-description: 빠르게 시작하는 데 도움이 되는 빠른 시작 및 빠른 시작을 소개합니다.
...
회전 집합은 개체를 회전하는 실제 동작을 시뮬레이션하여 검사합니다. 회전 집합을 사용하면 임의 각도에서 항목을 보고 모든 각도에서 주요 시각적 세부 사항을 얻을 수 있습니다. 회전 집합은 360도 보기 환경을 시뮬레이션합니다. Dynamic Media Classic는 뷰어에서 항목을 회전할 수 있는 1 차원 스핀 세트와 뷰어에서 항목을 회전하고 뒤집을 수 있는 2 차원 스핀 세트를 제공합니다. 또한 사용자는 몇 번의 마우스 클릭만으로 "형식에 상관없이" 확대/축소 및 패닝할 수 있습니다. 이렇게 하면 사용자가 특정 뷰포인트에서 더 자세히 항목을 검사할 수 있습니다.
...
이 스핀 세트 빠른 시작은 Dynamic Media Classic의 스핀 세트 기술을 사용하여 빠르게 시작하고 실행되도록 고안되었습니다. 1-7 단계를 수행하십시오. 각 단계 끝에는 필요한 경우 자세한 정보를 확인할 수 있는 항목 제목의 상호 참조가 있습니다.

**1. 이미지 만들기 및 업로드**

최소한 1 차원 스핀 세트의 경우 8-12 개의 샷이 필요하고 2 차원 스핀 세트의 경우 16-24가 필요합니다. 항목을 회전하고 뒤집는 듯한 인상을 주려면 일정한 간격으로 촬영해야 합니다. 예를 들어 1차원 회전 집합에 12개 촬영이 포함된 경우 각 촬영에 대해 항목을 30도(360/12) 회전합니다.
...
스핀 세트를 게시하면 다이내믹 미디어 클래식 서버에 게시되므로 웹 사이트나 애플리케이션에 동적으로 전달할 수 있습니다. Dynamic Media 이미지 서버에서 웹 사이트나 애플리케이션으로 스핀 세트를 호출하는 URL 문자열을 활성화합니다.

To publish a Spin Set, mark it for publish by selecting the **Mark for Publish** icon beside its name in the Browse Panel. 글로벌 탐색 막대에서 **게시를** 클릭하여 게시를 시작합니다. On the Publish screen, click **Start Publish**.

[회전 집합 게시](publishing-spin-set.md#publishing-a-spin-set)를 참조하십시오.

**7. 회전 집합을 웹 페이지에 연결**

Dynamic Media Classic는 스핀 집합에 대한 URL 설명선 문자열을 만들고 이를 게시한 후에 활성화합니다. [미리 보기] 화면에서 이러한 URL을 복사할 수 있습니다.

스핀 세트를 선택한 다음 **미리 보기를**&#x200B;클릭합니다. [미리 보기] 화면이 열립니다. 회전 집합 뷰어 사전 설정을 선택합니다. 그런 다음 URL **복사를**&#x200B;클릭합니다.

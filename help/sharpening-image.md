...
이미지를 선명하게 하려면 롤오버 **[!UICONTROL 편집]** 단추를 선택하고 **[!UICONTROL 선명하게]**&#x200B;를 선택하거나 [세부 사항 보기]에서 [찾아보기] 패널에서 연 다음 **[!UICONTROL 선명하게]**&#x200B;를 선택합니다. 선명하게 하기 명령이 있는 [선명도 편집기] 페이지가 열립니다. 명령을 선택한 다음 **[!UICONTROL 저장]**&#x200B;을 선택합니다.
...
| 언샵 마스크 | `op_usm` | `amount`,  `radius`,  `threshold`,  `monochrome`<br><br>`amount`: 필터 강도 계수(실수 0...5)<br><br>`radius`: 커널 반경을 픽셀 단위로 필터링(실수 0...250)  <br><br>`threshold`: 필터 임계값 수준(int 0...255)<br><br>`monochrome`: 각 색상  `0` 구성 요소를 개별적으로 언샵 마스크(unsharp-mask)하도록 설정하고, 언샵 마스크 이미지 밝기(강도) `1` 로 설정합니다. | `op_usm=1,1,10,0` |
...
**[!UICONTROL 재샘플링]** 메뉴를 선택하고 옵션을 선택합니다. 이미지를 다운샘플링할 때 다음 선택 사항은 이미지를 선명하게 합니다.

* **[!UICONTROL 없음]**  - 재결합을 해제합니다.

* **[!UICONTROL Bilinear]**  - 가장 빠른 재샘플링 방법 일부 앨리어싱 가공물들은 눈에 띄습니다.

* **[!UICONTROL 쌍입방]**  - 이미지 서버에서 CPU 사용을 증가시키지만 눈에 띄는 앨리어싱 가공물이 적은 선명한 이미지를 생성합니다.

* **[!UICONTROL 선명하게2]**  - 쌍입방 ****&#x200B;보다 약간 더 선명하게 결과를 생성하지만 이미지 서버의 CPU 비용은 더 높습니다.

* **[!UICONTROL 3선형]**  - 사용 가능한 경우 더 높은 해상도와 낮은 해상도를 모두 사용합니다. 앨리어싱이 문제인 경우에만 권장됩니다. 이 방법을 사용하면 고주파수 데이터가 감소하기 때문에 JPEG 크기가 줄어듭니다.
...
크기 범주에서 모든 이미지에 대해 하나의 사전 설정을 사용하는 경우 회사 관리자는 웹 코드를 변경하지 않고 해당 이미지 사전 설정의 정의를 업데이트하고 다시 게시할 수 있으며 해당 서식을 사용하는 모든 이미지에 영향을 미칠 수 있습니다. 사이트에서 고유 크기당 하나의 이미지 사전 설정을 사용하는 것이 좋습니다. 이미지 사전 설정을 추가하려면 전역 탐색 막대에서 **[!UICONTROL 설정]** > **[!UICONTROL 응용 프로그램 설정]** > **[!UICONTROL 이미지 사전 설정]**&#x200B;으로 이동합니다. 그런 다음 **[!UICONTROL 추가]**&#x200B;를 선택하거나 **[!UICONTROL 편집]**&#x200B;을 선택하여 기존 사전 설정을 변경합니다. 유일한 필수 필드는 사전 설정 이름입니다. 하지만 모든 사전 설정에 약간의 선명하게 하기를 포함하는 것이 가장 좋습니다.
...
회사의 기본 선명하게 하기 옵션을 설정하려면 **[!UICONTROL 설정]** > **[!UICONTROL 응용 프로그램 설정]** > **[!UICONTROL 게시 설정]** > **[!UICONTROL 이미지 서버]**&#x200B;로 이동하십시오. 기본 샘플링 모드를 **[!UICONTROL Sharp2]**&#x200B;로 설정하면 다운샘플링 시 항상 이미지가 선명하게 표시됩니다.
